    ...
    drive)
        ...
        case "$fixture" in
            ubuntu-26.04-dracut-systemd)
                ...
                guest_grub_disable_assignment='GRUB_CMDLINE_LINUX_DEFAULT="$GRUB_CMDLINE_LINUX_DEFAULT sart=0"'
                guest_grub_disable_prepare="$guest_sudo -k $guest_mkdir -p /etc/default/grub.d"
                guest_grub_disable_write="printf '%s\\n' '$guest_grub_disable_assignment' | $guest_sudo -k /usr/bin/$guest_tee /etc/default/grub.d/99-sart-vm-disable.cfg >/dev/null"
                guest_grub_disable_remove="$guest_sudo -k $guest_remove -f /etc/default/grub.d/99-sart-vm-disable.cfg"
                ;;
            fedora-44-dracut-systemd)
                ...
                guest_grub_disable_assignment='GRUB_CMDLINE_LINUX="$GRUB_CMDLINE_LINUX sart=0"'
                guest_grub_disable_prepare="$guest_sudo -k /usr/bin/$guest_copy -a /etc/default/grub /etc/default/grub.sart-vm-save"
                guest_grub_disable_write="printf '%s\\n' '$guest_grub_disable_assignment' | $guest_sudo -k /usr/bin/$guest_tee -a /etc/default/grub >/dev/null"
                guest_grub_disable_remove="$guest_sudo -k /usr/bin/$guest_move /etc/default/grub.sart-vm-save /etc/default/grub"
                ;;
        esac
        ...
        require_unchanged_display_count() {
            ...
            actual=$(count_log 'SART_LIFECYCLE_V1|event=display-acquired')
            [[ "$actual" == "$expected" ]]
        }
        ...
        privileged_step "$guest_sudo -k /mnt/sart-transport/sart $guest_install apply --confirm-host sart-vm" \
            'sart install apply: installed' 1800
        ...
        privileged_step "$guest_sudo -k $guest_sh -c 'test \"\$(cat /proc/1/comm)\" = systemd; root_source=\$(findmnt -n -o SOURCE /); case \"\$root_source\" in $guest_dev/mapper/*) ;; *) exit 1;; esac; test \"\$(/usr/bin/$guest_systemctl show systemd-ask-password-console.service -p ExecMainStartTimestampMonotonic --value)\" != 0; ! pgrep -x sart; /usr/bin/sart $guest_install status; unset root_source'" \
        ...
        privileged_step "$guest_sudo -k $guest_sh -c 'set -eu; unit=/usr/lib/systemd/system/sart-start.service; /usr/bin/$guest_copy -a \"\$unit\" \"\$unit.sart-vm-save\"; /usr/bin/sed \"s|^ExecStart=.*|ExecStart=/usr/bin/false|\" \"\$unit.sart-vm-save\" > \"\$unit\"; /usr/bin/$guest_chmod 0644 \"\$unit\"'" \
            SART_VM_RECOVERY_FAILURE_UNIT_STAGED_V1
        privileged_step "$guest_sudo -k /usr/bin/$guest_dracut --force $guest_initramfs.sart-vm-fail \$(uname -r)" \
            SART_VM_RECOVERY_FAILURE_IMAGE_BUILT_V1 1800
        privileged_step "$guest_sudo -k /usr/bin/$guest_move /usr/lib/systemd/system/sart-start.service.sart-vm-save /usr/lib/systemd/system/sart-start.service" \
            SART_VM_RECOVERY_FAILURE_UNIT_RESTORED_V1
        ...
        # accepted the stock unlock prompt, acquired no Sart display, mounted
        ...
        privileged_step "$guest_grub_disable_prepare" \
            SART_VM_RECOVERY_DISABLE_DIR_V1
