...
EOF
    ...
    drive)
        ...
        unlock_sart_root() {
            ...
            # Sart owns tty0 and deliberately suppresses the stock serial
        ...
        privileged_step "$guest_sudo -k $guest_sh -c '$guest_mkdir -p /var/tmp/sart-kernel-initramfs && cd /var/tmp/sart-kernel-initramfs && /usr/bin/lsinitrd --unpack $guest_new_initramfs && /usr/bin/cmp /mnt/sart-transport/sart usr/bin/sart && test -x usr/lib/systemd/systemd && test -f usr/lib/systemd/system/sart-start.service && cd / && $guest_rm -rf /var/tmp/sart-kernel-initramfs'" \
        ...
        privileged_step "$guest_sudo -k $guest_sh -c 'test \"\$(uname -r)\" = $new_kernel; root_source=\$(findmnt -n -o SOURCE /); case \"\$root_source\" in $guest_dev/mapper/*) ;; *) exit 1;; esac; $guest_crypt status \"\$root_source\" >/dev/null; test ! -e $guest_transport; test -z \"\$(find /sys/class/net -mindepth 1 -maxdepth 1 ! -name lo -print -quit)\"; /usr/bin/sart $guest_install status; unset root_source'" \
            SART_VM_KERNEL_UPDATE_NEW_KERNEL_BOOTED_V1
        privileged_step "$guest_sudo -k $guest_sh -c '$guest_mkdir -p /var/tmp/sart-kernel-reboot-check && cd /var/tmp/sart-kernel-reboot-check && /usr/bin/lsinitrd --unpack $guest_new_initramfs && /usr/bin/cmp /usr/bin/sart usr/bin/sart && cd / && $guest_rm -rf /var/tmp/sart-kernel-reboot-check; cd $package_cache; /usr/bin/sha256sum -c SHA256SUMS'" \
