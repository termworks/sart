#!/usr/bin/env bash
# TEST INFRASTRUCTURE ONLY. Installed Ubuntu one-ELF transaction proof.
...
EOF
    ...
    drive)
        ...
        unlock_stock_root() {
            ...
            # The prompt is printed before systemd-ask-password has completed
            # taking control of the console.  The independently proven stock
            # boot verifier uses this same settling interval; sending keys as
            # soon as grep sees the first byte can lose the entire sequence.
            sleep 10
            qmp_type_secret
            qmp_key ret
        ...
        unlock_bootart_root() {
            ...
            # The installed daemon deliberately suppresses the stock serial
            # prompt. Do not guess when its real ask-password request exists:
            # require the centered black Bootart password box first. The box
            # is rendered before the password reader is guaranteed to be
            # attached, so preserve the lifecycle lane's proven settling
            # interval before injecting input.
            wait_bootart_password_screen
            sleep 7
            qmp_type_secret
            qmp_key ret
        ...
        privileged_step "$guest_sudo -k $guest_sh -c '$guest_mkdir -p /var/tmp/bootart-initramfs-check && cd /var/tmp/bootart-initramfs-check && /usr/bin/lsinitrd --unpack $guest_initramfs && /usr/bin/cmp /mnt/bootart-transport/bootart usr/bin/bootart && cd / && $guest_rm -rf /var/tmp/bootart-initramfs-check'" \
        ...
        privileged_step "$guest_sudo -k $guest_sh -c '$guest_mkdir -p /var/tmp/bootart-initramfs-check && cd /var/tmp/bootart-initramfs-check && /usr/bin/lsinitrd --unpack $guest_initramfs && /usr/bin/cmp /usr/bin/bootart usr/bin/bootart && cd / && $guest_rm -rf /var/tmp/bootart-initramfs-check'" \
