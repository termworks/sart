...
EOF
    ...
    drive)
        ...
        unlock_sart_root() {
            ...
            # require the centered black Sart password box first. The box
        ...
        privileged_step "$guest_sudo -k $guest_sh -c '$guest_mkdir -p /var/tmp/sart-initramfs-check && cd /var/tmp/sart-initramfs-check && /usr/bin/lsinitrd --unpack $guest_initramfs && /usr/bin/cmp /mnt/sart-transport/sart usr/bin/sart && cd / && $guest_rm -rf /var/tmp/sart-initramfs-check'" \
        ...
        privileged_step "$guest_sudo -k $guest_sh -c '$guest_mkdir -p /var/tmp/sart-initramfs-check && cd /var/tmp/sart-initramfs-check && /usr/bin/lsinitrd --unpack $guest_initramfs && /usr/bin/cmp /usr/bin/sart usr/bin/sart && cd / && $guest_rm -rf /var/tmp/sart-initramfs-check'" \
