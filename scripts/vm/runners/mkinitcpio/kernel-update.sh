...
EOF
    ...
    drive)
        ...
        guest_package_cache='/''var/cache/sart-kernel-update'
        ...
        guest_test_entry=/etc/grub.d/42_sart_vm_kernel_update
        ...
        guest_entry_write="/usr/bin/sed -e 's@/initramfs-linux[.]img[.]sart-known-good@/initramfs-linux-lts.img@g' -e 's@/vmlinuz-linux @/vmlinuz-linux-lts @g' -e 's/Sart known-good/Sart kernel-update/g' -e 's/sart-known-good/sart-kernel-update/g' /etc/grub.d/41_sart_known_good | $guest_sudo -n /usr/bin/$guest_tee $guest_test_entry >/dev/null"
        ...
        qmp_sart_screendump() {
            local output="$run_dir/kernel-update-sart.ppm" response return_count
            response=$(printf '%s\n%s\n' '{"execute":"qmp_capabilities"}' \
                "{\"execute\":\"screendump\",\"arguments\":{\"filename\":\"$output\"}}" |
                timeout --signal=TERM --kill-after=1s 5s socat - "UNIX-CONNECT:$run_dir/qmp.sock")
            [[ "$response" != *'"error"'* ]]
        ...
        sart_prompt_visible() {
            tail -c 3072000 -- "$run_dir/kernel-update-sart.ppm" | od -An -v -tu1 | awk '
        ...
        wait_sart_prompt() {
            ...
            while (( elapsed < 120 )); do
                if qmp_sart_screendump && sart_prompt_visible; then return 0; fi
                sleep 1; ((elapsed += 1))
            done
            return 1
        }
        ...
        privileged_step "$guest_sudo -n /usr/bin/pacman -U --noconfirm $guest_package_cache/$package_file" \
            SART_VM_KERNEL_UPDATE_PACKAGES_INSTALLED_V1 1200
        ...
        privileged_step "$guest_sudo -n $guest_sh -c 'work=$guest_var_tmp/sart-kernel-initramfs; $guest_remove -rf \"\$work\"; $guest_mkdir \"\$work\"; cd \"\$work\"; /usr/bin/lsinitcpio -x $new_image; cmp /mnt/sart-transport/sart usr/bin/sart; grep -Fq sart:mkinitcpio-plymouth-native-v1 usr/bin/plymouth; test -x hooks/sart; $guest_remove -rf \"\$work\"; unset work'" \
            SART_VM_KERNEL_UPDATE_INITRAMFS_HASH_V1
        privileged_step "$guest_entry_write" \
            SART_VM_KERNEL_UPDATE_BOOT_ENTRY_STAGED_V1
        privileged_step "$guest_sudo -n /usr/bin/$guest_chmod 0755 $guest_test_entry" \
            SART_VM_KERNEL_UPDATE_BOOT_ENTRY_MODE_V1
        privileged_step "$guest_sudo -n $guest_grub_update -o $guest_grub_cfg" \
            SART_VM_KERNEL_UPDATE_BOOT_ENTRY_GENERATED_V1
        privileged_step "$guest_sudo -n $guest_grub_reboot sart-kernel-update" \
            SART_VM_KERNEL_UPDATE_BOOT_ENTRY_SELECTED_V1
        ...
        privileged_step "$guest_sudo -n $guest_sh -c 'set -eu; test \"\$(uname -r)\" = $new_kernel; root_source=\$(findmnt -n -o SOURCE /); case \"\$root_source\" in $guest_dev/mapper/*) ;; *) exit 1;; esac; crypt_source=\$(lsblk -rno PATH,TYPE -s \"\$root_source\" | while read -r path kind; do if test \"\$kind\" = crypt; then printf \"%s\\n\" \"\$path\"; break; fi; done); test -n \"\$crypt_source\"; /usr/bin/$guest_crypt status \"\$crypt_source\" | grep -Eq \"type:[[:space:]]+LUKS2\"; test ! -e $guest_transport; test -z \"\$(find /sys/class/net -mindepth 1 -maxdepth 1 ! -name lo -print -quit)\"; /usr/bin/sart $guest_install status; unset root_source crypt_source'" \
            SART_VM_KERNEL_UPDATE_NEW_KERNEL_BOOTED_V1
        privileged_step "$guest_sudo -n $guest_sh -c 'work=$guest_var_tmp/sart-kernel-reboot-check; $guest_remove -rf \"\$work\"; $guest_mkdir \"\$work\"; cd \"\$work\"; /usr/bin/lsinitcpio -x $new_image; cmp /usr/bin/sart usr/bin/sart; $guest_remove -rf \"\$work\"; cd $guest_package_cache; /usr/bin/sha256sum -c SHA256SUMS; unset work'" \
            SART_VM_KERNEL_UPDATE_REBOOT_HASH_V1
        privileged_step "$guest_sudo -n $guest_remove -f $guest_test_entry" \
            SART_VM_KERNEL_UPDATE_BOOT_ENTRY_REMOVED_V1
        privileged_step "$guest_sudo -n $guest_grub_update -o $guest_grub_cfg" \
            SART_VM_KERNEL_UPDATE_GRUB_RESTORED_V1
