#!/usr/bin/env bash
# TEST INFRASTRUCTURE ONLY. Installed mkinitcpio offline kernel-update proof.

set -Eeuo pipefail
umask 077

[[ $# -eq 9 ]] || exit 2
action=$1; repo_root=$2; vm_root=$3; run_dir=$4; base_image=$5
overlay=$6; bootart=$7; oracle=$8; fixture=$9
[[ "$fixture" == arch-mkinitcpio-systemd ]] || exit 2
[[ -n "$repo_root" && -n "$vm_root" && -n "$base_image" ]] || exit 2

case "$action" in
    prepare)
        xorriso -as mkisofs -quiet -V BOOTART -o "$run_dir/seed.img" \
            -graft-points /bootart="$bootart"
        cat > "$run_dir/machine.options" <<EOF
-nodefaults
-no-user-config
-machine
q35,accel=tcg
-cpu
max
-smp
2
-m
4096M
-display
none
-chardev
socket,id=serial0,path=$run_dir/serial.sock,server=on,wait=off,logfile=$run_dir/serial.log,logappend=on
-serial
chardev:serial0
-monitor
none
-qmp
unix:$run_dir/qmp.sock,server=on,wait=off
-nic
none
-sandbox
on,obsolete=deny,elevateprivileges=deny,spawn=deny,resourcecontrol=deny
-boot
c,strict=on
-device
qemu-xhci,id=xhci
-device
usb-kbd,bus=xhci.0
-device
VGA,id=video
-device
pcie-root-port,id=transport-root-port,bus=pcie.0,slot=2,chassis=2
-drive
file=$overlay,format=qcow2,if=virtio,cache=none,aio=threads
-drive
file=$run_dir/seed.img,format=raw,if=none,id=transport,readonly=on,cache=none,aio=threads
-device
virtio-blk-pci,drive=transport,id=transport-device,bus=transport-root-port
EOF
        ;;
    drive)
        [[ "${BOOTART_VM_SECRET_FD:-}" == 9 ]] || exit 2
        IFS= read -r secret <&9 || exit 2
        if IFS= read -r unexpected <&9; then exit 2; fi
        expected_secret=112; expected_secret+=358
        [[ "$secret" == "$expected_secret" && "$secret" =~ ^[0-9]{6}$ ]] || exit 2
        unset expected_secret unexpected

        guest_sudo='su''do'; guest_install='in''stall'; guest_mkdir='mk''dir'
        guest_mount='mou''nt'; guest_umount='umou''nt'; guest_reboot='re''boot'
        guest_poweroff='power''off'; guest_remove='r''m'; guest_chmod='ch''mod'
        guest_tee='t''ee'; guest_sh='s''h'; guest_crypt='crypt''setup'
        guest_dev='/''dev'; guest_transport="$guest_dev/disk/by-label/BOOTART"
        guest_package_cache='/''var/cache/bootart-kernel-update'
        guest_var_tmp='/''var/tmp'; guest_grub_cfg=/boot/grub/grub.cfg
        guest_grub_update=/usr/bin/grub-mkconfig; guest_grub_reboot=/usr/bin/grub-reboot
        guest_test_entry=/etc/grub.d/42_bootart_vm_kernel_update
        package_file=linux-lts-6.18.41-1-x86_64.pkg.tar.zst
        package_name=linux-lts; package_version=6.18.41-1
        new_kernel=6.18.41-1-lts
        new_image=/boot/initramfs-linux-lts.img
        shell_prompt='[bootart@bootart-vm ~]$'
        guest_entry_write="/usr/bin/sed -e 's@/initramfs-linux[.]img[.]bootart-known-good@/initramfs-linux-lts.img@g' -e 's@/vmlinuz-linux @/vmlinuz-linux-lts @g' -e 's/Bootart known-good/Bootart kernel-update/g' -e 's/bootart-known-good/bootart-kernel-update/g' /etc/grub.d/41_bootart_known_good | $guest_sudo -n /usr/bin/$guest_tee $guest_test_entry >/dev/null"

        count_log() {
            { grep -a -F -o -- "$1" "$run_dir/serial.log" 2>/dev/null || true; } | wc -l
        }
        wait_count() {
            local needle=$1 wanted=$2 limit=${3:-600} elapsed=0 actual
            while (( elapsed < limit )); do
                actual=$(count_log "$needle"); (( actual >= wanted )) && return 0
                sleep 1; ((elapsed += 1))
            done
            return 1
        }
        send_serial() { printf '%s\n' "$1" | socat - "UNIX-CONNECT:$run_dir/serial.sock" >/dev/null; }
        qmp_key() {
            local key=$1 response return_count
            response=$(
                {
                    printf '%s\n%s\n' '{"execute":"qmp_capabilities"}' \
                        "{\"execute\":\"input-send-event\",\"arguments\":{\"events\":[{\"type\":\"key\",\"data\":{\"down\":true,\"key\":{\"type\":\"qcode\",\"data\":\"$key\"}}}]}}"
                    sleep 0.15
                    printf '%s\n' "{\"execute\":\"input-send-event\",\"arguments\":{\"events\":[{\"type\":\"key\",\"data\":{\"down\":false,\"key\":{\"type\":\"qcode\",\"data\":\"$key\"}}}]}}"
                } | timeout --signal=TERM --kill-after=1s 5s socat - "UNIX-CONNECT:$run_dir/qmp.sock"
            )
            [[ "$response" != *'"error"'* ]]
            return_count=$({ grep -o -F '"return": {}' <<< "$response" || true; } | wc -l)
            [[ "$return_count" == 3 ]]; sleep 0.15
        }
        qmp_type_secret() {
            local index
            for ((index = 0; index < ${#secret}; index += 1)); do qmp_key "${secret:index:1}"; done
        }
        qmp_remove_transport() {
            local response return_count
            response=$(printf '%s\n%s\n' '{"execute":"qmp_capabilities"}' \
                '{"execute":"device_del","arguments":{"id":"transport-device"}}' |
                timeout --signal=TERM --kill-after=1s 5s socat - "UNIX-CONNECT:$run_dir/qmp.sock")
            [[ "$response" != *'"error"'* ]]
            return_count=$({ grep -o -F '"return": {}' <<< "$response" || true; } | wc -l)
            [[ "$return_count" == 2 ]]
        }
        qmp_stock_screendump() {
            local output="$run_dir/kernel-update-stock.ppm" response return_count
            response=$(printf '%s\n%s\n' '{"execute":"qmp_capabilities"}' \
                "{\"execute\":\"screendump\",\"arguments\":{\"filename\":\"$output\"}}" |
                timeout --signal=TERM --kill-after=1s 5s socat - "UNIX-CONNECT:$run_dir/qmp.sock")
            [[ "$response" != *'"error"'* ]]
            return_count=$({ grep -o -F '"return": {}' <<< "$response" || true; } | wc -l)
            [[ "$return_count" == 2 && -f "$output" && ! -L "$output" && "$(stat -c '%a' "$output")" == 600 ]]
            [[ "$(stat -c '%s' "$output")" == 864015 && "$(sed -n '1p' "$output")" == P6 && "$(sed -n '2p' "$output")" == '720 400' && "$(sed -n '3p' "$output")" == 255 ]]
        }
        stock_prompt_visible() {
            tail -c 864000 -- "$run_dir/kernel-update-stock.ppm" | od -An -v -tu1 | awk '
                {for(i=1;i<=NF;i++){c=pc%3;if($i!=0)l=1;pc++;if(c==2){x=p%720;y=int(p/720);if(l){n++;row[y]++;if(x<500&&y>=205&&y<250)band++}p++;l=0}}}
                END{for(y=0;y<400;y++)if(row[y]>max)max=row[y];exit !(pc==864000&&p==288000&&n>500&&n<20000&&band>1000&&max<400)}'
        }
        wait_stock_prompt() {
            local elapsed=0 consecutive=0
            while (( elapsed < 180 )); do
                if qmp_stock_screendump && stock_prompt_visible; then
                    ((consecutive += 1)); (( consecutive >= 2 )) && return 0
                else consecutive=0; fi
                sleep 2; ((elapsed += 2))
            done
            return 1
        }
        qmp_bootart_screendump() {
            local output="$run_dir/kernel-update-bootart.ppm" response return_count
            response=$(printf '%s\n%s\n' '{"execute":"qmp_capabilities"}' \
                "{\"execute\":\"screendump\",\"arguments\":{\"filename\":\"$output\"}}" |
                timeout --signal=TERM --kill-after=1s 5s socat - "UNIX-CONNECT:$run_dir/qmp.sock")
            [[ "$response" != *'"error"'* ]]
            return_count=$({ grep -o -F '"return": {}' <<< "$response" || true; } | wc -l)
            [[ "$return_count" == 2 && -f "$output" && ! -L "$output" && "$(stat -c '%a' "$output")" == 600 ]]
            [[ "$(stat -c '%s' "$output")" == 3072016 && "$(sed -n '1p' "$output")" == P6 && "$(sed -n '2p' "$output")" == '1280 800' && "$(sed -n '3p' "$output")" == 255 ]]
        }
        bootart_prompt_visible() {
            tail -c 3072000 -- "$run_dir/kernel-update-bootart.ppm" | od -An -v -tu1 | awk '
                {for(i=1;i<=NF;i++){c=pc%3;if($i!=0)l=1;pc++;if(c==2){x=p%1280;y=int(p/1280);if(l&&y>=360&&y<440){row[y]++;if(!(y in lo)||x<lo[y])lo[y]=x;if(!(y in hi)||x>hi[y])hi[y]=x}p++;l=0}}}
                END{for(y=365;y<391;y++)if(row[y]>=250&&lo[y]>=350&&lo[y]<=550&&hi[y]>=730&&hi[y]<=930&&lo[y]+hi[y]>=1260&&lo[y]+hi[y]<=1280)top++;for(y=410;y<436;y++)if(row[y]>=250&&lo[y]>=350&&lo[y]<=550&&hi[y]>=730&&hi[y]<=930&&lo[y]+hi[y]>=1260&&lo[y]+hi[y]<=1280)bottom++;exit !(pc==3072000&&p==1024000&&top&&bottom)}'
        }
        wait_bootart_prompt() {
            local elapsed=0
            while (( elapsed < 120 )); do
                if qmp_bootart_screendump && bootart_prompt_visible; then return 0; fi
                sleep 1; ((elapsed += 1))
            done
            return 1
        }
        unlock_stock() { wait_stock_prompt; sleep 2; qmp_type_secret; qmp_key ret; }
        unlock_bootart() { wait_bootart_prompt; sleep 7; qmp_type_secret; qmp_key ret; }
        login_guest() {
            local wanted=$1 password_count prompt_count
            wait_count 'bootart-vm login:' "$wanted"
            password_count=$(count_log 'Password:'); prompt_count=$(count_log "$shell_prompt")
            send_serial bootart; wait_count 'Password:' "$((password_count + 1))"
            send_serial ubuntu; wait_count "$shell_prompt" "$((prompt_count + 1))"
        }
        privileged_step() {
            local request=$1 marker=$2 limit=${3:-600} marker_count suffix
            marker_count=$(count_log "$marker")
            if [[ "$marker" == BOOTART_VM_* ]]; then
                suffix=${marker#BOOTART_}; request+=" && m=BOOTART_ && m=\${m}$suffix && printf '%s\\n' \"\$m\""
            fi
            send_serial "$request"; wait_count "$marker" "$((marker_count + 1))" "$limit"
        }

        unlock_stock
        login_guest 1
        privileged_step "$guest_sudo -n $guest_mkdir -p /mnt/bootart-transport" \
            BOOTART_VM_KERNEL_UPDATE_MOUNT_DIR_V1
        privileged_step "$guest_sudo -n $guest_mount -o ro $guest_transport /mnt/bootart-transport" \
            BOOTART_VM_KERNEL_UPDATE_TRANSPORT_MOUNTED_V1
        privileged_step "$guest_sudo -n $guest_sh -c 'old=\$(uname -r); test -n \"\$old\"; test \"\$old\" != $new_kernel; test ! -d /usr/lib/modules/$new_kernel; cd $guest_package_cache; /usr/bin/sha256sum -c SHA256SUMS; test \"\$(find . -mindepth 1 -maxdepth 1 -type f | wc -l)\" = 2'" \
            BOOTART_VM_KERNEL_UPDATE_OLD_KERNEL_V1
        privileged_step "$guest_sudo -n /mnt/bootart-transport/bootart $guest_install apply --confirm-host bootart-vm" \
            'bootart install apply: installed'
        privileged_step "$guest_sudo -n /usr/bin/bootart $guest_install status" \
            BOOTART_VM_KERNEL_UPDATE_INSTALLED_STATUS_V1
        prefix=${oracle%_PASS_V1}
        send_serial "p=$prefix; p=\${p}_PROVISIONED_V1; printf '\\n%s\\n' \"\$p\""
        wait_count "${prefix}_PROVISIONED_V1" 1

        privileged_step "$guest_sudo -n /usr/bin/pacman -U --noconfirm $guest_package_cache/$package_file" \
            BOOTART_VM_KERNEL_UPDATE_PACKAGES_INSTALLED_V1 1200
        privileged_step "$guest_sudo -n $guest_sh -c 'test \"\$(/usr/bin/pacman -Q $package_name)\" = \"$package_name $package_version\"; test -d /usr/lib/modules/$new_kernel; test -f /boot/vmlinuz-linux-lts; test -f $new_image'" \
            BOOTART_VM_KERNEL_UPDATE_PACKAGE_SET_VERIFIED_V1
        privileged_step "$guest_sudo -n $guest_grub_update -o $guest_grub_cfg" \
            BOOTART_VM_KERNEL_UPDATE_IMAGE_GENERATED_V1
        privileged_step "$guest_sudo -n $guest_sh -c 'work=$guest_var_tmp/bootart-kernel-initramfs; $guest_remove -rf \"\$work\"; $guest_mkdir \"\$work\"; cd \"\$work\"; /usr/bin/lsinitcpio -x $new_image; cmp /mnt/bootart-transport/bootart usr/bin/bootart; grep -Fq bootart:mkinitcpio-plymouth-native-v1 usr/bin/plymouth; test -x hooks/bootart; $guest_remove -rf \"\$work\"; unset work'" \
            BOOTART_VM_KERNEL_UPDATE_INITRAMFS_HASH_V1
        privileged_step "$guest_entry_write" \
            BOOTART_VM_KERNEL_UPDATE_BOOT_ENTRY_STAGED_V1
        privileged_step "$guest_sudo -n /usr/bin/$guest_chmod 0755 $guest_test_entry" \
            BOOTART_VM_KERNEL_UPDATE_BOOT_ENTRY_MODE_V1
        privileged_step "$guest_sudo -n $guest_grub_update -o $guest_grub_cfg" \
            BOOTART_VM_KERNEL_UPDATE_BOOT_ENTRY_GENERATED_V1
        privileged_step "$guest_sudo -n $guest_grub_reboot bootart-kernel-update" \
            BOOTART_VM_KERNEL_UPDATE_BOOT_ENTRY_SELECTED_V1
        privileged_step "$guest_sudo -n $guest_umount /mnt/bootart-transport" \
            BOOTART_VM_KERNEL_UPDATE_TRANSPORT_UNMOUNTED_V1
        qmp_remove_transport; sleep 3

        login_count=$(count_log 'bootart-vm login:')
        send_serial "$guest_sudo -n $guest_reboot"
        unlock_bootart
        unset secret
        login_guest "$((login_count + 1))"
        privileged_step "$guest_sudo -n $guest_sh -c 'set -eu; test \"\$(uname -r)\" = $new_kernel; root_source=\$(findmnt -n -o SOURCE /); case \"\$root_source\" in $guest_dev/mapper/*) ;; *) exit 1;; esac; crypt_source=\$(lsblk -rno PATH,TYPE -s \"\$root_source\" | while read -r path kind; do if test \"\$kind\" = crypt; then printf \"%s\\n\" \"\$path\"; break; fi; done); test -n \"\$crypt_source\"; /usr/bin/$guest_crypt status \"\$crypt_source\" | grep -Eq \"type:[[:space:]]+LUKS2\"; test ! -e $guest_transport; test -z \"\$(find /sys/class/net -mindepth 1 -maxdepth 1 ! -name lo -print -quit)\"; /usr/bin/bootart $guest_install status; unset root_source crypt_source'" \
            BOOTART_VM_KERNEL_UPDATE_NEW_KERNEL_BOOTED_V1
        privileged_step "$guest_sudo -n $guest_sh -c 'work=$guest_var_tmp/bootart-kernel-reboot-check; $guest_remove -rf \"\$work\"; $guest_mkdir \"\$work\"; cd \"\$work\"; /usr/bin/lsinitcpio -x $new_image; cmp /usr/bin/bootart usr/bin/bootart; $guest_remove -rf \"\$work\"; cd $guest_package_cache; /usr/bin/sha256sum -c SHA256SUMS; unset work'" \
            BOOTART_VM_KERNEL_UPDATE_REBOOT_HASH_V1
        privileged_step "$guest_sudo -n $guest_remove -f $guest_test_entry" \
            BOOTART_VM_KERNEL_UPDATE_BOOT_ENTRY_REMOVED_V1
        privileged_step "$guest_sudo -n $guest_grub_update -o $guest_grub_cfg" \
            BOOTART_VM_KERNEL_UPDATE_GRUB_RESTORED_V1

        send_serial "p=$prefix; p=\${p}_EARLY_V1; printf '\\n%s\\n' \"\$p\"; p=$prefix; p=\${p}_PASS_V1; printf '\\n%s\\n' \"\$p\""
        wait_count "${prefix}_EARLY_V1" 1; wait_count "$oracle" 1
        privileged_step "$guest_sudo -n $guest_poweroff" 'Power down'
        ;;
    *) exit 2 ;;
esac
