...
EOF
    ...
    drive)
        ...
        wait_sart_password() {
            ...
            while (( elapsed < 90 )); do
                if qmp_screendump "$name" "$refresh" && \
                    require_sart_password "$run_dir/$name"; then
                    return 0
                fi
                refresh=yes
                sleep 1; ((elapsed += 1))
        ...
        require_sart_layout_in_any() {
            local frame
            for frame in "$@"; do
                if require_sart_layout "$frame"; then return 0; fi
            done
            return 1
        }
        ...
        unlock_stock
        login_guest 1
        privileged_step "$guest_sudo -n $guest_mkdir -p /mnt/sart-transport" SART_VM_LIFECYCLE_MOUNT_DIR_V1
        privileged_step "$guest_sudo -n $guest_mount -o ro $guest_transport /mnt/sart-transport" SART_VM_LIFECYCLE_TRANSPORT_MOUNTED_V1
        ...
        privileged_step "$guest_sudo -n $guest_umount /mnt/sart-transport" SART_VM_LIFECYCLE_TRANSPORT_UNMOUNTED_V1
        qmp_remove_transport; sleep 3

        login_count=$(count_log 'sart-vm login:')
        send_serial "$guest_sudo -n $guest_reboot"
        wait_sart_password lifecycle-before-1.ppm
        sleep 1; qmp_screendump lifecycle-before-2.ppm
        sleep 1; qmp_screendump lifecycle-before-3.ppm
        require_sart_layout "$run_dir/lifecycle-before-1.ppm"
        ...
        for frame in "${after_frames[@]}"; do require_sart_alternate_frame "$frame"; done
        require_sart_layout_in_any "${after_frames[@]}"
        require_distinct_frames "${after_frames[@]}"

        login_guest "$((login_count + 1))"
        privileged_step "$guest_sudo -n $guest_sh -c 'set -eu; test \"\$(cat /proc/1/comm)\" = systemd; test \"\$(cat /sys/class/tty/tty0/active)\" = tty1; ! pgrep -x sart; test ! -e $guest_transport; tree=/var/tmp/sart-lifecycle-initramfs; $guest_rm -rf \"\$tree\"; $guest_mkdir \"\$tree\"; cd \"\$tree\"; /usr/bin/lsinitcpio -x $guest_image; cmp /usr/bin/sart usr/bin/sart; grep -Fq sart:mkinitcpio-plymouth-native-v1 usr/bin/plymouth; test -x hooks/sart; $guest_rm -rf \"\$tree\"; /usr/bin/sart $guest_install status'" SART_VM_LIFECYCLE_HANDOFF_VERIFIED_V1
