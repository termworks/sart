...
EOF
    ...
    drive)
        ...
        guest_install_hook=/usr/lib/initcpio/in''stall/bootart
        ...
        wait_bootart_prompt() {
            local elapsed=0 image="$run_dir/uninstall-bootart.ppm"
            while (( elapsed < 90 )); do
                if qmp_screendump "$image" 1280 800 && bootart_prompt_visible "$image"; then return 0; fi
                sleep 1; ((elapsed += 1))
            done
            return 1
        }
        ...
        privileged_step "$guest_sudo -n /usr/bin/bootart $guest_install uninstall --confirm-host bootart-vm" \
            'bootart install uninstall:' 900
        privileged_step "$guest_sudo -n $guest_sh -c 'set -eu; test ! -e /usr/bin/bootart; test ! -e $guest_manifest; test ! -e $guest_install_hook; test ! -e /usr/lib/initcpio/hooks/bootart; test ! -e /usr/lib/bootart/mkinitcpio-plymouth; test ! -e /usr/lib/systemd/system/bootart-start.service; test ! -e /usr/lib/systemd/system/bootart-show.service; test ! -e /usr/lib/systemd/system/bootart-switch-root.service; test ! -e /usr/lib/systemd/system/bootart-quit.service; test ! -e /usr/lib/systemd/system/bootart-quit-wait.service; test ! -e /usr/lib/systemd/system/systemd-ask-password-console.service.d/50-bootart.conf; test ! -e /etc/grub.d/41_bootart_known_good; test ! -e $guest_image.bootart-known-good; ! grep -Eq \"(^|[[:space:]])bootart([[:space:]]|$)\" /etc/mkinitcpio.conf; ! grep -a -Fq bootart-known-good /boot/grub/grub.cfg; work=$guest_var_tmp/bootart-uninstall-initramfs; $guest_remove -rf \"\$work\"; $guest_mkdir \"\$work\"; cd \"\$work\"; /usr/bin/lsinitcpio -x $guest_image; test -e hooks/encrypt; test ! -e hooks/bootart; test ! -e usr/bin/bootart; test ! -e usr/bin/plymouth; ! find . -iname \"*bootart*\" -print -quit | grep -q .; $guest_remove -rf \"\$work\"; /usr/bin/sha256sum $guest_image > $guest_var_tmp/bootart-uninstall-clean.sha256'" \
        ...
        unset secret
        login_guest "$((login_count + 1))"
        privileged_step "$guest_sudo -n $guest_sh -c 'set -eu; test \"\$(cat /proc/1/comm)\" = systemd; root_source=\$(findmnt -n -o SOURCE /); case \"\$root_source\" in $guest_dev/mapper/*) ;; *) exit 1;; esac; ! pgrep -x bootart; test ! -e /usr/bin/bootart; test ! -e $guest_manifest; /usr/bin/sha256sum -c $guest_var_tmp/bootart-uninstall-clean.sha256; $guest_remove -f $guest_var_tmp/bootart-uninstall-clean.sha256; unset root_source'" \
