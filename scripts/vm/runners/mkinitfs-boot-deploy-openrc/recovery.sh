...
EOF
    ...
    drive)
        ...
        type_secret
        login_admin
        root_step "sha256sum /boot/initramfs /boot/loader/entries/pmos.conf > $guest_var_tmp/bootart-recovery-baseline.sha256" \
            BOOTART_VM_MKINITFS_BOOT_DEPLOY_RECOVERY_BASELINE_V1
        ...
        # Kill the ordinary production ELF only after its durable journal is
        # ready but before mutation begins.  The next ordinary invocation must
        # refuse new work until explicit recovery restores the exact baseline.
        root_step "set -eu; journal=/.bootart-installer-journal.v1; manifest=/.bootart-install-manifest.v1; $transport_path/bootart $guest_install apply --confirm-host bootart-pmos <$guest_tty >$guest_tty 2>&1 & pid=\$!; elapsed=0; while kill -0 \"\$pid\" 2>/dev/null && ! { test -r \"\$journal\" && grep -q '^phase[[:space:]]ready\$' \"\$journal\"; } && test \"\$elapsed\" -lt 12000; do sleep 0.05; elapsed=\$((elapsed + 1)); done; test -r \"\$journal\"; grep -q '^phase[[:space:]]ready\$' \"\$journal\"; kill -STOP \"\$pid\"; test ! -e \"\$manifest\"; kill -KILL \"\$pid\"; set +e; wait \"\$pid\"; set -e; sleep 1; sync" \
            BOOTART_VM_MKINITFS_BOOT_DEPLOY_RECOVERY_PRODUCTION_CRASH_V1 1200
        root_step "$transport_path/bootart $guest_install recover --confirm-host bootart-pmos" \
            'bootart install recover: rolled-back'
        root_step "sha256sum -c $guest_var_tmp/bootart-recovery-baseline.sha256 && test ! -e /.bootart-installer-journal.v1 && test ! -e /.bootart-install-manifest.v1 && test ! -e /usr/bin/bootart" \
            BOOTART_VM_MKINITFS_BOOT_DEPLOY_RECOVERY_CRASH_ROLLED_BACK_V1
        ...
        root_step "$guest_copy /boot/loader/entries/pmos.conf $guest_var_tmp/bootart-recovery-active.conf && $guest_copy /boot/loader/entries/bootart-known-good.conf /boot/loader/entries/pmos.conf && sync" \
            BOOTART_VM_MKINITFS_BOOT_DEPLOY_RECOVERY_KNOWN_GOOD_SELECTED_V1
        login_count=$(count_log 'bootart-pmos login:')
        ...
        wait_count_for 'bootart-pmos login:' "$((login_count + 1))" 600
        login_admin
        root_step "grep -Eq '(^|[[:space:]])bootart=0([[:space:]]|$)' /proc/cmdline && test ! -S /run/bootart/control.sock && $guest_copy $guest_var_tmp/bootart-recovery-active.conf /boot/loader/entries/pmos.conf && sync && /usr/bin/bootart $guest_install status" \
            BOOTART_VM_MKINITFS_BOOT_DEPLOY_RECOVERY_KNOWN_GOOD_BOOT_V1
