#!/usr/bin/env bash
# TEST INFRASTRUCTURE ONLY. postmarketOS ARM64 Sart/OpenRC lifecycle proof.
...
EOF
    ...
    drive)
        ...
        screen="$run_dir/postmarketos-lifecycle-screen.ppm"
        ...
        if [[ "$fixture" == "${expected_fixture}-systemd" ]]; then
            ...
            root_step "test -f $fairphone_fixture && test \"\$(sha256sum $fairphone_fixture | awk '{ print \$1 }')\" = $fairphone_sha && $guest_copy $fairphone_fixture /usr/share/deviceinfo/deviceinfo && test -b $guest_phone_boot && grep -Fq 'androidboot.slot_suffix=_a' /proc/cmdline && phone_raw_before=\$(sha256sum $guest_phone_boot | awk '{ print \$1 }')" \
                SART_VM_FAIRPHONE_FP6_LIFECYCLE_FIXTURE_V1
        fi
        ...
        if [[ "$fixture" == "${expected_fixture}-systemd" ]]; then
            root_step "phone_raw_after=\$(sha256sum $guest_phone_boot | awk '{ print \$1 }'); test \"\$phone_raw_after\" != \"\$phone_raw_before\" && printf '%s\n' \"\$phone_raw_after\" > $guest_phone_installed && grep -Fxq \"deviceinfo_flash_kernel_on_update='false'\" $guest_deviceinfo && awk -F '\\t' '\$1 == \"file\" && \$2 == \"2f6574632f646576696365696e666f\" && \$3 == \"600\" { found++ } END { exit found != 1 }' $guest_manifest" \
                SART_VM_FAIRPHONE_FP6_LIFECYCLE_RAW_INSTALLED_V1
        fi
        ...
        wait_screen sart
        cp "$screen" "$run_dir/postmarketos-lifecycle-frame-1.ppm"
        first_frame=$(sha256sum "$screen" | awk '{ print $1 }')
        sleep 0.35
        # The first frame was already fully classified as Sart.  Repeating
        # the multi-megabyte pixel classifier here can consume most of the
        # client's bounded askpass deadline under ARM TCG before the test types
        # the passphrase.  A bounded QMP capture plus a distinct frame hash is
        # the animation proof; do not delay the real credential interaction.
        monitor_screen
        cp "$screen" "$run_dir/postmarketos-lifecycle-frame-2.ppm"
        second_frame=$(sha256sum "$screen" | awk '{ print $1 }')
        [[ "$first_frame" =~ ^[0-9a-f]{64}$ && "$second_frame" =~ ^[0-9a-f]{64}$ && "$first_frame" != "$second_frame" ]]
        type_secret
        wait_count_for 'sart-pmos login:' "$((login_count + 1))" 600
        login_admin
        ...
        if [[ "$fixture" == "${expected_fixture}-systemd" ]]; then
            root_step "test \"\$(sha256sum $guest_phone_boot | awk '{ print \$1 }')\" = \"\$(cat $guest_phone_installed)\" && grep -Fxq \"deviceinfo_flash_kernel_on_update='false'\" $guest_deviceinfo" \
                SART_VM_FAIRPHONE_FP6_LIFECYCLE_RAW_REBOOT_V1
            root_step "test ! -S /run/sart/control.sock && ! /usr/bin/sart ping >/dev/null 2>&1 && test \"\$(cat /proc/1/comm)\" = systemd && test -f /usr/lib/systemd/system/sart-quit.service && test -L /etc/systemd/system/multi-user.target.wants/sart-quit.service" \
                SART_VM_MKINITFS_BOOT_DEPLOY_SYSTEMD_HANDOFF_COMPLETE_V1
        else
            root_step "test ! -S /run/sart/control.sock && ! /usr/bin/sart ping >/dev/null 2>&1 && test -L /etc/runlevels/boot/sart && test -L /etc/runlevels/default/sart-quit" \
                SART_VM_MKINITFS_BOOT_DEPLOY_OPENRC_HANDOFF_COMPLETE_V1
        fi
