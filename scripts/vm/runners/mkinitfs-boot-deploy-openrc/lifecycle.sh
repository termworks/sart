#!/usr/bin/env bash
# TEST INFRASTRUCTURE ONLY. postmarketOS ARM64 Bootart/OpenRC lifecycle proof.
...
EOF
    ...
    drive)
        ...
        root_step "test ! -S /run/bootart/control.sock && ! /usr/bin/bootart ping >/dev/null 2>&1 && test -L /etc/runlevels/boot/bootart && test -L /etc/runlevels/default/bootart-quit" \
            BOOTART_VM_MKINITFS_BOOT_DEPLOY_OPENRC_HANDOFF_COMPLETE_V1
