...
[[ -f "$serial" && ! -L "$serial" ]] || {
    printf 'sart-vm: adapter serial transcript is missing or unsafe: %s\n' "$serial" >&2
    exit 1
}
[[ "$pass_oracle" =~ ^SART_VM_[A-Z0-9_]+_PASS_V1$ ]] || {
    printf 'sart-vm: adapter PASS oracle is invalid\n' >&2
    exit 2
}
...
require_one_exact_occurrence() {
    ...
    [[ "$exact_count" -eq 1 && "$occurrence_count" -eq 1 ]] || {
        printf 'sart-vm: adapter transcript requires one exact %s oracle and no extra occurrences: %s\n' \
            "$label" "$serial" >&2
        exit 1
    }
}
...
[[ "$(count_fixed_occurrences "$fail_oracle")" -eq 0 ]] || {
    printf 'sart-vm: adapter transcript contains a FAIL oracle: %s\n' "$serial" >&2
    exit 1
}
...
[[ "$provisioned_line" =~ ^[1-9][0-9]*$ && "$early_line" =~ ^[1-9][0-9]*$ && \
   ...
   "$provisioned_line" -lt "$early_line" && "$early_line" -lt "$pass_line" ]] || {
    printf 'sart-vm: adapter evidence must be ordered provisioned, early-initramfs, then PASS: %s\n' \
        "$serial" >&2
    exit 1
}
