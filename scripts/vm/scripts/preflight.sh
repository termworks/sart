...
tools=(bash awk basename cat chmod cp cpio curl date dirname find findmnt grep gzip head id install jq ln mkdir mkfifo mktemp od prlimit qemu-img readelf readlink rm sed sha256sum sleep socat sort stat tail tar timeout touch tr truncate wc xorriso)
