#!/usr/bin/env bash
# TEST INFRASTRUCTURE ONLY. Exact BIOS/QEMU policy for the stock Arch proof.
...
[[ $# -eq 7 ]] || vm_die \
    'usage: check-arch-stock-command.sh REPO VM RUN ARGS BASE OVERLAY SERIAL_LOG'
...
[[ "$args_file" == "$run_dir/stock-qemu.args" && -f "$args_file" && ! -L "$args_file" &&
   "$(vm_stat_mode "$args_file")" == 600 ]] || vm_die 'Arch stock args are unsafe'
[[ "$base" == "$vm_root/cache/provisioned/"* && -f "$base" && ! -L "$base" &&
   "$(vm_stat_mode "$base")" == 400 ]] || vm_die 'Arch stock base is not sealed'
[[ "$overlay" == "$run_dir/stock-overlay.qcow2" && -f "$overlay" && ! -L "$overlay" &&
   "$(vm_stat_mode "$overlay")" == 600 ]] || vm_die 'Arch stock overlay is unsafe'
vm_assert_qcow2_backing_file "$overlay" "$base"
[[ "$serial_log" == "$run_dir/stock-serial.raw" && -f "$serial_log" && ! -L "$serial_log" &&
   "$(vm_stat_mode "$serial_log")" == 600 && "$(vm_stat_size "$serial_log")" == 0 ]] ||
    vm_die 'Arch stock serial evidence must begin empty'
[[ ! -e "$run_dir/serial.sock" && ! -L "$run_dir/serial.sock" &&
   ! -e "$run_dir/qmp.sock" && ! -L "$run_dir/qmp.sock" ]] ||
    vm_die 'Arch stock control sockets exist before launch'
...
[[ ${#actual[@]} -eq ${#expected[@]} ]] || vm_die 'Arch stock QEMU argument count differs'
for index in "${!expected[@]}"; do
    vm_reject_newline "${actual[index]}" 'Arch stock QEMU argument'
    [[ "${actual[index]}" == "${expected[index]}" ]] ||
        vm_die "Arch stock QEMU argument $index differs from policy"
done
! grep -F -- "$base" "$args_file" >/dev/null ||
    vm_die 'sealed Arch base may be reachable only through its private overlay'
...
! grep -E -- '(^|,)hostfwd=|(^|,)guestfwd=|^-virtfs$|^-fsdev$|^virtio-9p([,-]|$)|^vhost-user-fs([,-]|$)|^usb-host([,-]|$)' \
    "$args_file" >/dev/null || vm_die 'Arch stock share or passthrough denied'
sha256sum "$args_file" | awk '{ print $1 }' > "$run_dir/stock-qemu.policy.sha256"
chmod 0600 -- "$run_dir/stock-qemu.policy.sha256"
printf 'bootart-vm: Arch stock QEMU command policy PASS\n'
