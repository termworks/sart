...
[[ -f "$serial" && ! -L "$serial" ]] || {
    printf 'bootart-vm: lifecycle serial transcript is missing or unsafe: %s\n' "$serial" >&2
    exit 1
}
[[ -n "$pass_marker" && -n "$fail_marker" && "$pass_marker" != "$fail_marker" && \
   "$pass_marker" != *$'\n'* && "$pass_marker" != *$'\r'* && \
   "$fail_marker" != *$'\n'* && "$fail_marker" != *$'\r'* ]] || {
    printf 'bootart-vm: lifecycle serial markers are invalid\n' >&2
    exit 2
}
...
[[ "$exact_pass_lines" -eq 1 && "$pass_occurrences" -eq 1 ]] || {
    printf 'bootart-vm: final lifecycle serial transcript requires one exact PASS line and no additional PASS occurrences: %s\n' \
        "$serial" >&2
    exit 1
}
[[ "$fail_occurrences" -eq 0 ]] || {
    printf 'bootart-vm: final lifecycle serial transcript has %s FAIL marker occurrences; expected zero: %s\n' \
