#!/usr/bin/env bash
# TEST INFRASTRUCTURE ONLY. Exact QEMU policy for the Arch builder boot.
...
[[ $# -eq 10 ]] || vm_die \
    'usage: check-arch-provision-command.sh REPO VM RUN ARGS SOURCE SOURCE_OVERLAY TARGET SEED SERIAL_FIFO SERIAL_LOG'
...
[[ "$args_file" == "$run_dir/provision-qemu.args" && -f "$args_file" && ! -L "$args_file" &&
   "$(vm_stat_mode "$args_file")" == 600 ]] || vm_die 'Arch provision args are unsafe'
[[ "$source_base" == "$vm_root/cache/images/"* && -f "$source_base" && ! -L "$source_base" &&
   "$(vm_stat_mode "$source_base")" == 400 ]] || vm_die 'Arch source is not sealed'
for writable in "$source_overlay" "$target_disk"; do
    [[ "$writable" == "$run_dir/"* && -f "$writable" && ! -L "$writable" &&
       "$(vm_stat_mode "$writable")" == 600 ]] || vm_die "unsafe Arch writable disk: $writable"
done
[[ "$seed_iso" == "$run_dir/seed.iso" && -f "$seed_iso" && ! -L "$seed_iso" &&
   "$(vm_stat_mode "$seed_iso")" == 400 ]] || vm_die 'Arch seed is unsafe'
[[ "$serial_fifo" == "$run_dir/serial.fifo" && -p "$serial_fifo" && ! -L "$serial_fifo" &&
   "$(vm_stat_mode "$serial_fifo")" == 600 ]] || vm_die 'Arch serial FIFO is unsafe'
[[ "$serial_log" == "$run_dir/provision-serial.log" && -f "$serial_log" && ! -L "$serial_log" &&
   "$(vm_stat_mode "$serial_log")" == 600 && "$(vm_stat_size "$serial_log")" == 0 ]] ||
    vm_die 'Arch serial evidence must begin empty'
[[ ! -e "$run_dir/qmp.sock" && ! -L "$run_dir/qmp.sock" ]] ||
    vm_die 'Arch provisioning QMP path exists before launch'
vm_assert_qcow2_backing_file "$source_overlay" "$source_base"
qemu_img="$(vm_resolve_qemu_img "${QEMU_IMG:-qemu-img}")"
target_info="$("$qemu_img" info --output=json -- "$target_disk")" || vm_die 'cannot inspect Arch target'
jq -e '.format == "qcow2" and (has("backing-filename") | not)' <<< "$target_info" >/dev/null ||
    vm_die 'Arch target must not have a backing file'
...
[[ ${#actual[@]} -eq ${#expected[@]} ]] || vm_die 'Arch provision QEMU argument count differs'
for index in "${!expected[@]}"; do
    vm_reject_newline "${actual[index]}" 'Arch provision QEMU argument'
    [[ "${actual[index]}" != *'/dev/'* ]] || vm_die 'host device path is forbidden'
    [[ "${actual[index]}" == "${expected[index]}" ]] ||
        vm_die "Arch provision QEMU argument $index differs from policy"
done
