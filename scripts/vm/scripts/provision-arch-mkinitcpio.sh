#!/usr/bin/env bash
# TEST INFRASTRUCTURE ONLY. Install encrypted Arch onto a private QEMU disk.
...
[[ $# -eq 9 ]] || vm_die \
    'usage: provision-arch-mkinitcpio.sh REPO VM LOCK PACKAGE_LOCK SOURCE_ID DERIVED_ID QEMU QEMU_IMG TIMEOUT'
...
[[ "$provision_timeout" =~ ^[1-9][0-9]{2,4}$ && "$provision_timeout" -le 7200 ]] ||
    vm_die 'Arch provisioning timeout must be 100..7200 seconds'
...
IFS='|' read -r _ source_status source_url source_sha source_format source_arch \
    source_filename _ _ source_bytes source_virtual source_run_cap source_file_cap \
    source_log_cap _ <<< "$source_record"
[[ "$source_status" == verified && "$source_format" == qcow2 &&
   "$source_arch" == x86_64 &&
   "$source_filename" == Arch-Linux-x86_64-cloudimg-20260715.556894.qcow2 &&
   "$source_virtual" == 21474836480 ]] ||
    vm_die 'Arch builder source differs from the reviewed cloud-image contract'
...
[[ "$derived_status" == derived && "$derived_format" == qcow2 &&
   ...
   "$derived_virtual" == 25769803776 ]] ||
    vm_die 'Arch derived-image lock row is inconsistent with its source'
...
[[ -f "$source_image" && ! -L "$source_image" &&
   "$(vm_stat_mode "$source_image")" == 400 ]] ||
    vm_die 'Arch cloud source is missing or unsealed'
vm_assert_owned "$source_image"
vm_assert_file_size_exact "$source_image" "$source_bytes" 'Arch cloud source'
printf '%s  %s\n' "$source_sha" "$source_image" | sha256sum --check --status - ||
    vm_die 'Arch cloud source checksum mismatch'
QEMU_IMG="$qemu_img" vm_assert_qcow2_virtual_size "$source_image" "$source_virtual" >/dev/null

kernel_package_dir="$vm_root/cache/kernel-packages/arch-6.18.41-lts-x86_64"
...
expected_kernel_packages="$(awk -F '|' \
    '$0 !~ /^#/ && NF && $10 == "arch-mkinitcpio-systemd" { print $6 }' \
    "$package_lock" | sort)"
actual_kernel_packages="$(find "$kernel_package_dir" -xdev -mindepth 1 -maxdepth 1 \
    -type f -printf '%f\n' | sort)"
[[ "$actual_kernel_packages" == "$expected_kernel_packages" ]] ||
    vm_die 'offline Arch kernel package cache contains an unexpected or missing file'
package_path="$kernel_package_dir/linux-lts-6.18.41-1-x86_64.pkg.tar.zst"
package_record="$(awk -F '|' '$1 == "arch-6.18.41-lts-x86_64" { print; found++ } END { if (found != 1) exit 1 }' "$package_lock")" ||
    vm_die 'reviewed Arch kernel package row is absent'
IFS='|' read -r _ _ _ package_sha package_bytes package_filename _ _ _ _ <<< "$package_record"
[[ "$package_filename" == linux-lts-6.18.41-1-x86_64.pkg.tar.zst &&
   -f "$package_path" && ! -L "$package_path" && "$(vm_stat_mode "$package_path")" == 400 ]] ||
    vm_die 'offline Arch kernel package is missing or unsealed'
vm_assert_owned "$package_path"
vm_assert_file_size_exact "$package_path" "$package_bytes" 'offline Arch kernel package'
printf '%s  %s\n' "$package_sha" "$package_path" | sha256sum --check --status - ||
    vm_die 'offline Arch kernel package checksum mismatch'
...
template="$repo_root/scripts/vm/arch-mkinitcpio-builder.user-data.in"
metadata="$repo_root/scripts/vm/arch-mkinitcpio-builder.meta-data"
for input in "$template" "$metadata"; do
    [[ -f "$input" && ! -L "$input" && -O "$input" ]] ||
        vm_die "unsafe Arch NoCloud source: $input"
    mode="$(vm_stat_mode "$input")"
    (( (8#$mode & 0022) == 0 )) || vm_die "writable Arch NoCloud source: $input"
done
template_sha="$(sha256sum "$template" | awk '{ print $1 }')"
[[ "$(grep -Fxc "      bootart_luks='__BOOTART_VM_LUKS_PASSPHRASE__'" "$template")" == 1 ]] ||
    vm_die 'Arch NoCloud template must contain one exact LUKS marker'
...
if [[ -e "$base" || -L "$base" || -e "$lineage" || -L "$lineage" ]]; then
    [[ -f "$base" && ! -L "$base" && -f "$lineage" && ! -L "$lineage" &&
       "$(vm_stat_mode "$base")" == 400 && "$(vm_stat_mode "$lineage")" == 400 ]] ||
        vm_die 'partial or unsafe Arch provisioned cache entry exists'
    ...
    [[ "$(sed -n 's/^schema=//p' "$lineage")" == BOOTART_ARCH_PROVISIONED_V1 &&
       ...
       "$(sed -n 's/^template_sha256=//p' "$lineage")" == "$template_sha" ]] ||
        vm_die 'cached Arch provisioned lineage is stale'
    base_sha="$(sed -n 's/^base_sha256=//p' "$lineage")"
    [[ "$base_sha" =~ ^[0-9a-f]{64}$ ]] || vm_die 'cached Arch base hash is invalid'
    printf '%s  %s\n' "$base_sha" "$base" | sha256sum --check --status - ||
        vm_die 'cached Arch base differs from lineage'
    QEMU_IMG="$qemu_img" vm_assert_qcow2_virtual_size "$base" "$derived_virtual" "$derived_virtual" >/dev/null
    printf 'bootart-vm: validated cached, stock-unverified Arch base: %s\n' "$base"
    exit 0
fi
...
target_disk="$run_dir/arch-target.qcow2"
...
capture_pid=; qemu_pid=; published=no
cleanup() {
    status=$?
...
xorriso -as mkisofs -quiet -volid CIDATA -joliet -rock -graft-points \
    -output "$seed_iso" "user-data=$user_data" "meta-data=$meta_data" \
    "kernel-packages/SHA256SUMS=$kernel_package_manifest" \
    "kernel-packages/$package_filename=$package_path" >/dev/null 2>&1
chmod 0400 -- "$seed_iso"

vm_assert_executable_identity "$qemu_img" "$qemu_img_identity" 'Arch provisioning QEMU_IMG'
...
"$qemu_img" resize "$source_overlay" 8G >/dev/null
QEMU_IMG="$qemu_img" vm_assert_qcow2_virtual_size "$source_overlay" 8589934592 8589934592 >/dev/null
"$qemu_img" create -f qcow2 "$target_disk" 24G >/dev/null
...
QEMU="$qemu" QEMU_IMG="$qemu_img" bash "$SCRIPT_DIR/check-arch-provision-command.sh" \
...
capture_pid=$!
vm_assert_executable_identity "$qemu" "$qemu_identity" 'Arch provisioning QEMU'
printf 'bootart-vm: installing encrypted Arch inside QEMU (timeout %ss)\n' "$provision_timeout"
set +e
...
rm -f -- "$serial_fifo"
[[ $qemu_status -eq 0 ]] || vm_die "Arch provisioning QEMU failed or timed out: status $qemu_status"
[[ ! -e "$serial_overflow" ]] || vm_die 'Arch provisioning serial output exceeded its bound'
[[ "$(grep -a -Fc BOOTART_VM_ARCH_PROVISION_PASS_V1 "$serial_log" || true)" == 1 ]] ||
    vm_die "Arch provisioning completion oracle is absent; inspect $serial_log"
! grep -a -Fq BOOTART_VM_ARCH_PROVISION_FAIL_V1 "$serial_log" ||
    vm_die "Arch provisioning reported failure; inspect $serial_log"

rm -f -- "$user_data" "$meta_data" "$seed_iso" "$kernel_package_manifest" "$source_overlay"
unset luks_passphrase
vm_assert_file_size_at_most "$target_disk" "$max_file_bytes" 'Arch provisioned qcow2'
QEMU_IMG="$qemu_img" vm_assert_qcow2_virtual_size "$target_disk" "$derived_virtual" "$derived_virtual" >/dev/null
"$qemu_img" check -q "$target_disk" || vm_die 'Arch provisioned qcow2 failed structural validation'
printf '%s  %s\n' "$source_sha" "$source_image" | sha256sum --check --status - ||
    vm_die 'Arch immutable source changed during provisioning'
...
for retained in "$serial_log" "$args_file"; do
    if printf '%s\n' "$secret_pattern" | grep -a -F -q --devices=skip -f - -- "$retained"; then
        vm_die 'synthetic LUKS passphrase entered retained Arch provisioning evidence'
    fi
done
...
printf '%s\n' \
    'schema=BOOTART_ARCH_PROVISIONED_V1' 'status=PROVISIONED_UNVERIFIED' \
    ...
    'provision_oracle=BOOTART_VM_ARCH_PROVISION_PASS_V1' > "$lineage_tmp"
chmod 0400 -- "$target_disk" "$lineage_tmp"
ln -- "$target_disk" "$base" || vm_die 'refusing to replace Arch provisioned base'
ln -- "$lineage_tmp" "$lineage" || { rm -f -- "$base"; vm_die 'refusing to replace Arch lineage'; }
rm -f -- "$lineage_tmp"
published=yes
printf 'bootart-vm: sealed stock-unverified encrypted Arch base: %s\n' "$base"
