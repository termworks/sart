...
if [[ -e "$base" || -L "$base" || -e "$lineage" || -L "$lineage" ]]; then
    ...
    printf 'sart-vm: validated cached, stock-unverified Arch base: %s\n' "$base"
    exit 0
fi
...
printf 'sart-vm: installing encrypted Arch inside QEMU (timeout %ss)\n' "$provision_timeout"
set +e
...
[[ "$(grep -a -Fc SART_VM_ARCH_PROVISION_PASS_V1 "$serial_log" || true)" == 1 ]] ||
    vm_die "Arch provisioning completion oracle is absent; inspect $serial_log"
! grep -a -Fq SART_VM_ARCH_PROVISION_FAIL_V1 "$serial_log" ||
...
printf '%s\n' \
    'schema=SART_ARCH_PROVISIONED_V1' 'status=PROVISIONED_UNVERIFIED' \
    ...
    'provision_oracle=SART_VM_ARCH_PROVISION_PASS_V1' > "$lineage_tmp"
...
rm -f -- "$lineage_tmp"
published=yes
printf 'sart-vm: sealed stock-unverified encrypted Arch base: %s\n' "$base"
