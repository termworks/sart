...
die() {
    printf 'sart-init-neutral: ERROR: %s\n' "$*" >&2
    exit 1
}
...
for surface in "$repo_root/Makefile" "$repo_root/flake.nix"; do
    [[ -f "$surface" && ! -L "$surface" ]] || die "link surface is missing or symlinked: $surface"
done
if unsafe_link="$(find "$repo_root/src" "$repo_root/include" -xdev -type l -print -quit)" &&
   [[ -n "$unsafe_link" ]]; then
...
source_hit="$(find "$repo_root/src" "$repo_root/include" -type f \
...
link_hit="$(grep -H -n -E -- \
    ...
    "$repo_root/Makefile" "$repo_root/flake.nix" \
    2>/dev/null || true)"
[[ -z "$link_hit" ]] || die "forbidden init-specific native link configuration: $link_hit"

printf 'sart-init-neutral: PASS: core has no systemd/D-Bus link or API binding\n'
