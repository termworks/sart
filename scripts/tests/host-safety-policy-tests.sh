...
new_fixture() {
    ...
    mkdir -p -- "$root/scripts" "$root/vm/scripts" "$root/vm/runners" \
...
printf '#!/usr/bin/env bash\n%s true\n' "do""as" > "$fixture/vm/scripts/bad.sh"
expect_rejected "$fixture"

fixture="$(new_fixture runner-tree)"
mkdir -p -- "$fixture/vm/runners/example"
printf '#!/usr/bin/env bash\n%s true\n' "do""as" > "$fixture/vm/runners/example/bad.sh"
expect_rejected "$fixture"
...
for device in disk/by-id/root loop0 dm-0 md0 zvol/pool/root; do
