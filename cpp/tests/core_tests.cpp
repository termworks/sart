...
#include "bootart/splash/client.hpp"
#include "bootart/splash/command.hpp"
...
#define CHECK(...)                                                                                                     \
    ...
    template <typename Function> void check_art_error(bootart::ArtErrorCode expected, Function &&function) {
        try {
            function();
        } catch (const bootart::ArtError &error) {
    ...
    void art_validation_and_layout() {
        const auto art = bootart::Art::parse("  foo  \r\n  bar\n");
        CHECK(art.width() == 5);
        CHECK(art.height() == 2);
        CHECK(art.cell(0, 0) == U' ');
        CHECK(art.cell(2, 0) == U'f');

        const auto unicode = bootart::Art::parse(" ▄▄██ \n ▀▀██ ");
        CHECK(unicode.width() == 5);
        CHECK(unicode.cell(1, 0) == U'▄');
        check_art_error(bootart::ArtErrorCode::empty, [] { bootart::Art::parse(""); });
        check_art_error(bootart::ArtErrorCode::no_visible_characters, [] { bootart::Art::parse("   \n  \n"); });
        check_art_error(bootart::ArtErrorCode::contains_tab, [] { bootart::Art::parse("hello\tworld"); });
        check_art_error(bootart::ArtErrorCode::contains_standalone_carriage_return,
                        [] { bootart::Art::parse("hello\rworld"); });
        check_art_error(bootart::ArtErrorCode::contains_control, [] { bootart::Art::parse("left\xE2\x80\xAEright"); });

        const auto centered = bootart::layout({10, 4}, {80, 24});
        CHECK(centered == bootart::Layout{0, 0, 10, 4, 35, 10});
        const auto cropped = bootart::layout({100, 30}, {80, 24});
        CHECK(cropped == bootart::Layout{10, 3, 80, 24, 0, 0});
    }

    void animation_is_deterministic() {
        CHECK(bootart::cell_hash(42, 10, 5) == bootart::cell_hash(42, 10, 5));
        CHECK(bootart::cell_hash(42, 10, 5) != bootart::cell_hash(43, 10, 5));
        CHECK(bootart::smoothstep(0.0F) == 0.0F);
        CHECK(bootart::smoothstep(0.5F) == 0.5F);
        CHECK(bootart::smoothstep(1.0F) == 1.0F);
        for (std::uint64_t seed = 0; seed < 10; ++seed) {
            const auto value = bootart::normalized_hash(seed, 3, 4);
            CHECK(value >= 0.0F && value <= 1.0F);
        }
        const auto art = bootart::Art::parse("X X\n XX");
        const bootart::AnimationMetadata metadata(art, 42);
    ...
    void renderer_matches_rust_goldens() {
        const auto art = bootart::Art::parse("  ____  \n / __ \\ \n/ /_/ / \n/_____/ ");
        const bootart::TerminalSize terminal_size{40, 10};
        const auto layout = bootart::layout(art.size(), {40, 10});
        const bootart::AnimationMetadata metadata(art, 42);
        ...
        for (std::size_t index = 0; index < points.size(); ++index) {
            const auto actual = bootart::generate_frame_bytes(art, metadata, layout,
                                                              {points[index], false, points[index] == 0.0F, true, 0});
            const auto expected = read_file(std::filesystem::path(BOOTART_SOURCE_ROOT) / "tests/golden" / names[index]);
            CHECK(actual == expected);
        }

        const auto pixel = bootart::Art::parse("X");
        bootart::BufferTerminal terminal({10, 5});
        bootart::render_final(terminal, pixel, nullptr, false);
    ...
    template <typename Function> void check_scene_error(bootart::SceneErrorCode expected, Function &&function) {
        try {
            function();
        } catch (const bootart::SceneError &error) {
            CHECK(error.code() == expected);
            return;
        }
    ...
    template <typename Function> void check_display_error(bootart::DisplayErrorCode expected, Function &&function) {
        try {
            function();
        } catch (const bootart::DisplayError &error) {
            CHECK(error.code() == expected);
            return;
        }
    ...
    void display_and_frame_engine_contracts() {
        check_scene_error(bootart::SceneErrorCode::empty_dimensions, [] { bootart::Dimensions(0, 24); });
        check_scene_error(bootart::SceneErrorCode::control_glyph, [] { bootart::Cell(U'\x1b'); });
        const std::array<std::string_view, 2> rows{"ab", "c"};
        auto scene = bootart::Scene::from_rows(rows);
        CHECK(scene.dimensions() == bootart::Dimensions(2, 2));
        CHECK(scene.get(1, 1)->glyph() == U' ');
        scene.set(1, 1, bootart::Cell(U'd'));
        CHECK(scene.get(1, 1)->glyph() == U'd');
        check_scene_error(bootart::SceneErrorCode::out_of_bounds, [&] { scene.set(2, 0, bootart::Cell()); });

        bootart::BufferBackend display(bootart::Dimensions(2, 1));
        const std::array<std::string_view, 1> display_row{"ok"};
        const auto display_scene = bootart::Scene::from_rows(display_row);
        check_display_error(bootart::DisplayErrorCode::invalid_state, [&] { display.render(display_scene); });
        display.acquire();
        CHECK(display.state() == bootart::DisplayState::hidden);
        display.show();
        display.show();
        display.render(display_scene);
        display.queue_input(bootart::InputEvent::bytes({0x1b}));
        ...
        CHECK(display.state() == bootart::DisplayState::details);
        CHECK(!display.poll_input(std::chrono::milliseconds(0)));
        display.queue_input(bootart::InputEvent::return_to_splash());
        auto returned = display.poll_input(std::chrono::milliseconds(0));
        CHECK(returned && returned->kind() == bootart::InputEvent::Kind::return_to_splash);
        ...
        CHECK(display.operations().front().kind == bootart::BufferOperation::Kind::acquire);
        CHECK(display.operations().back().kind == bootart::BufferOperation::Kind::restore);

        bootart::BufferBackend narrow(bootart::Dimensions(3, 1));
        narrow.acquire();
        narrow.show();
        check_display_error(bootart::DisplayErrorCode::sensitive_text_out_of_bounds,
                            [&] { narrow.render_sensitive_text(0, 0, "界界", {}); });

        const auto art = bootart::Art::parse("X");
        const bootart::FrameEngine engine(art, 42);
        const auto frame = engine.render({5, 3}, 0.5F, true, 0);
        CHECK(frame.dimensions() == bootart::Dimensions(5, 3));
        CHECK(frame.get(2, 1)->glyph() == U'X');
        CHECK(frame.get(2, 1)->style().foreground == bootart::Color::white);
        CHECK(*frame.get(0, 0) == bootart::Cell());
        check_scene_error(bootart::SceneErrorCode::empty_dimensions,
    ...
    struct FakeVtState {
        ...
        bootart::Dimensions dimensions{20, 6};
        termios terminal{};
        int kd_mode{1};
        int keyboard_mode{2};
    ...
    class FakeVtIo final : public bootart::VtIo {
      public:
        ...
        bootart::Dimensions dimensions(int) override { return state_.dimensions; }
        ...
        bootart::VtDeallocation disallocate(int, std::uint16_t number) override {
            state_.deallocations.push_back(number);
            return bootart::VtDeallocation::deallocated;
        }
    ...
    void text_vt_backend_contracts() {
        FakeVtState state;
        bootart::TextVtBackend backend(bootart::TextVtConfig::open_query(), std::make_unique<FakeVtIo>(state));
        backend.acquire();
        CHECK(backend.state() == bootart::DisplayState::hidden);
        ...
        backend.show();
        CHECK(backend.state() == bootart::DisplayState::splash);
        const std::array<std::string_view, 6> rows{"                    ", "                    ",
                                                   "        ok          ", "                    ",
                                                   "                    ", "                    "};
        backend.render(bootart::Scene::from_rows(rows));
        ...
        const std::array<std::uint8_t, 1> escape{0x1b};
        CHECK(input && input->equals_bytes(escape));
        state.dimensions = bootart::Dimensions(21, 6);
        auto resized = backend.poll_input(std::chrono::milliseconds(0));
        CHECK(resized && resized->kind() == bootart::InputEvent::Kind::resized);
        backend.details(true);
        CHECK(backend.state() == bootart::DisplayState::details);
        state.active = 7;
        auto returned = backend.poll_input(std::chrono::milliseconds(0));
        CHECK(returned && returned->kind() == bootart::InputEvent::Kind::return_to_splash);
        backend.details(false);
        backend.restore();
        CHECK(backend.state() == bootart::DisplayState::restored);
        ...
        bootart::TextVtBackend configured(bootart::TextVtConfig::configured(9),
                                          std::make_unique<FakeVtIo>(configured_state));
        configured.acquire();
    ...
    void splash_engine_contracts() {
        using namespace bootart::splash;
        const auto art = bootart::Art::parse("X");
        auto owned_backend = std::make_unique<bootart::BufferBackend>(bootart::Dimensions(30, 8));
        ...
        const auto encoded = bootart::encode_scene(backend->frames().back());
        ...
        CHECK(backend->state() == bootart::DisplayState::restored);
        CHECK(backend->operations().back().restore_mode == bootart::RestoreMode::retain_pixels);
    }

    void password_memory_and_input_contracts() {
        using namespace bootart::password;
        try {
    ...
    void systemd_password_contracts() {
        using namespace bootart::password;
        const auto root =
            std::filesystem::path(BOOTART_SOURCE_ROOT) / "target/cpp" / ("ask-test-" + std::to_string(getpid()));
        ...
        bootart::splash::SplashState prompt_state;
        static_cast<void>(prompt_state.apply(bootart::splash::MarkRunning{}));
    ...
    void digest_and_cpio_contracts() {
        CHECK(bootart::sha256("abc") == "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
        const auto elf_text = std::string_view("\x7f"
                                               "ELFfake");
        const auto hook_text = std::string_view("#!/bin/sh");
        const std::array inputs{
            bootart::integration::CpioInput{"usr/bin/bootart",
                                            std::as_bytes(std::span(elf_text.data(), elf_text.size())), 0755},
            bootart::integration::CpioInput{"usr/lib/dracut/modules.d/60bootart/module-setup.sh",
                                            std::as_bytes(std::span(hook_text.data(), hook_text.size())), 0755},
        };
        const auto archive = bootart::integration::build_cpio_archive(inputs);
        const auto entries = bootart::integration::parse_cpio_archive(archive);
        CHECK(entries.size() == 2);
        CHECK(entries[0].name == "usr/bin/bootart");
        const auto report = bootart::integration::inspect_candidate_archive(
            archive, bootart::sha256(elf_text), bootart::integration::AdapterId::dracut_systemd);
        CHECK(report.entries_count == 2);
        CHECK(report.candidate_digest == bootart::sha256(archive));
    }
    ...
    void native_password_transport_contracts() {
        ...
        bootart::splash::FileDescriptor first(carriers[0]);
        bootart::splash::FileDescriptor second(carriers[1]);
    ...
    void native_password_coordinator_contracts() {
        using namespace bootart::password;
        using namespace bootart::splash;
        const auto runtime = std::filesystem::path(BOOTART_SOURCE_ROOT) / "target/cpp" /
                             ("native-coordinator-" + std::to_string(getpid()));
        std::error_code ignored;
        ...
        const auto broken_runtime = std::filesystem::path(BOOTART_SOURCE_ROOT) / "target/cpp" /
    ...
    void embedded_and_cmdline_contracts() {
        CHECK(bootart::Art::parse(bootart::embedded::default_art).height() == 22);
        CHECK(bootart::Art::parse(bootart::embedded::small_art).height() == 3);
        CHECK(bootart::embedded::resource_set_version == 13);
        CHECK(bootart::embedded::default_config.ends_with('\n'));
        CHECK(bootart::embedded::default_config.contains("runtime_dir=/run/bootart\n"));
        CHECK(bootart::embedded::template_ids.size() == 37);
        std::set<std::string_view> names;
        std::set<std::string_view> paths;
        for (const auto id : bootart::embedded::template_ids) {
            const auto resource = bootart::embedded::template_resource(id);
            ...
            CHECK(names.insert(bootart::embedded::template_name(id)).second);
            CHECK(resource.materialization.path.starts_with('/'));
            CHECK(resource.materialization.path.find("/../") == std::string_view::npos);
            if (resource.materialization.kind == bootart::embedded::MaterializationKind::managed_snippet) {
                CHECK(resource.materialization.mode == 0);
                CHECK(!resource.materialization.insertion_point.empty());
                CHECK(resource.contents.contains("# bootart:begin"));
                CHECK(resource.contents.contains("# bootart:end"));
            } else {
            ...
            CHECK(!resource.contents.starts_with("\x7f"
                                                 "ELF"));
            CHECK(!resource.contents.contains("\nexec /usr/bin/bootart "));
        }
        CHECK(!bootart::cmdline::splash_disabled("quiet splash bootart=1"));
        CHECK(bootart::cmdline::splash_disabled("quiet bootart=0 splash"));
        CHECK(bootart::cmdline::splash_disabled("rd.bootart=0"));
        CHECK(!bootart::cmdline::splash_disabled("xbootart=0 bootart=00"));
        CHECK(!bootart::process_is_allowed(1));
        CHECK(bootart::process_is_allowed(2));

        CHECK(bootart::adapter_ids.size() == 8);
        CHECK(bootart::adapter_pairs().size() == 7);
        std::set<std::string_view> adapter_names;
        for (const auto id : bootart::adapter_ids) {
            const auto &metadata = bootart::adapter_metadata(id);
            CHECK(metadata.id == id);
            CHECK(adapter_names.insert(metadata.name).second);
            for (const auto resource : metadata.resources) {
                static_cast<void>(bootart::embedded::template_resource(resource));
            }
        }
        CHECK(bootart::adapter_pair(bootart::AdapterId::dracut_classic, bootart::AdapterId::openrc_real_root)->status ==
              bootart::SupportStatus::experimental_unproven);
        CHECK(bootart::adapter_pair(bootart::AdapterId::mkinitfs_boot_deploy, bootart::AdapterId::systemd_real_root)
                  ->status == bootart::SupportStatus::proven_supported);
        ...
        const std::string mkinitfs_source = "#!/bin/sh\nVERSION=3.14.0-r0\n\n"
                                            ...
                                            "\t\tfi\n\tdone\n\t$MOCK sync\n";
        const auto mkinitfs_patched = bootart::integration::patch_mkinitfs_init(mkinitfs_source);
        CHECK(mkinitfs_patched.has_value());
        CHECK(mkinitfs_patched->contains("# bootart:begin mkinitfs-early-v1"));
        CHECK(mkinitfs_patched->contains("# bootart:begin mkinitfs-handoff-v1"));
        CHECK(bootart::integration::patch_mkinitfs_init(*mkinitfs_patched) == mkinitfs_patched);

        const std::string boot_deploy_source = R"BOOTART(prefix
...
suffix
        ...
        const auto deploy_patched = bootart::integration::patch_boot_deploy_init_functions(
            boot_deploy_source, bootart::integration::reviewed_boot_deploy_initramfs_version);
        CHECK(deploy_patched.has_value());
        CHECK(deploy_patched->contains("# bootart:begin mkinitfs-boot-deploy-fde-v1"));
        CHECK(bootart::integration::patch_boot_deploy_init_functions(
                  *deploy_patched, bootart::integration::reviewed_boot_deploy_initramfs_version) == deploy_patched);

        const auto elf = static_elf_fixture();
        bootart::install::validate_static_elf(elf);
        auto interpreted = elf;
        interpreted[64] = std::byte{3};
        bool rejected_interpreter = false;
        try {
            bootart::install::validate_static_elf(interpreted);
        } catch (const std::runtime_error &) {
            rejected_interpreter = true;
        }
        CHECK(rejected_interpreter);
        const auto dracut_plan = bootart::install::build_install_plan(elf, bootart::AdapterId::dracut_systemd,
                                                                      bootart::AdapterId::systemd_real_root);
        ...
        CHECK(bootart::install::render_plan_human(dracut_plan, true).contains("status: READY"));
        CHECK(bootart::install::render_plan_json(dracut_plan, true).contains("\"actionable\":true"));
        const auto mkinitfs_plan = bootart::install::build_install_plan(elf, bootart::AdapterId::mkinitfs_busybox,
                                                                        bootart::AdapterId::openrc_real_root);
    ...
    void installer_transaction_contracts() {
        ...
        const auto source = std::string("/tmp/bootart-cpp-installer-") + std::to_string(getpid()) + "-XXXXXX";
        ...
        const auto elf = static_elf_fixture();
        const auto plan = bootart::install::build_install_plan(
            elf, bootart::AdapterId::dracut_systemd, bootart::AdapterId::systemd_real_root, false, root.string());
        bootart::install::Installer installer(root.string(), geteuid(), true);
        CHECK(installer.apply(plan) == bootart::install::ApplyOutcome::installed);
        const auto status = installer.status();
        CHECK(status.installed);
        CHECK(!status.recovery_required);
        CHECK(status.files.size() == 10);
        CHECK(std::ranges::all_of(
            status.files, [](const auto &file) { return file.state == bootart::install::FileStatusState::exact; }));
        CHECK(installer.apply(plan) == bootart::install::ApplyOutcome::already_current);

        const auto manifest = root / "var/lib/bootart/install/manifest.v1";
        ...
        const auto binary = root / "usr/bin/bootart";
        {
        ...
        CHECK(std::ranges::any_of(modified.files, [](const auto &file) {
            return file.state == bootart::install::FileStatusState::content_modified;
        }));
        {
            std::ofstream output(binary, std::ios::binary | std::ios::trunc);
        ...
        const auto patch_source = std::string("/tmp/bootart-cpp-patch-") + std::to_string(getpid()) + "-XXXXXX";
        ...
        const auto patch_plan =
            bootart::install::build_install_plan(elf, bootart::AdapterId::mkinitfs_busybox,
                                                 bootart::AdapterId::openrc_real_root, false, patch_root.string());
        bootart::install::Installer patch_installer(patch_root.string(), geteuid(), true);
        CHECK(patch_installer.apply(patch_plan) == bootart::install::ApplyOutcome::installed);
        CHECK(read_file(target).find("# bootart:begin mkinitfs-early-v1") != std::string::npos);
        ...
        std::fill(root_template.begin(), root_template.end(), '\0');
        const auto recovery_source = std::string("/tmp/bootart-cpp-recover-") + std::to_string(getpid()) + "-XXXXXX";
        ...
        const auto recovery_plan =
            bootart::install::build_install_plan(elf, bootart::AdapterId::dracut_systemd,
                                                 bootart::AdapterId::systemd_real_root, false, recovery_root.string());
        const auto child = fork();
        CHECK(child >= 0);
        if (child == 0) {
            try {
                bootart::install::Installer child_installer(recovery_root.string(), geteuid(), true);
        ...
        const auto journal = recovery_root / ".bootart-installer-journal.v1";
        bool ready = false;
        ...
        bootart::install::Installer recovery_installer(recovery_root.string(), geteuid(), true);
        CHECK(recovery_installer.status().recovery_required);
        CHECK(recovery_installer.recover() == bootart::install::RecoveryOutcome::rolled_back);
        CHECK(!recovery_installer.status().installed);
        CHECK(!std::filesystem::exists(recovery_root / "usr/bin/bootart"));
        CHECK(!std::filesystem::exists(journal));
        std::filesystem::remove_all(recovery_root);
    }

    template <typename Function> void check_state_error(bootart::splash::StateErrorCode expected, Function &&function) {
        try {
            function();
        } catch (const bootart::splash::StateError &error) {
            CHECK(error.code() == expected);
            return;
        }
    ...
    bootart::splash::SplashState running_state() {
        bootart::splash::SplashState state;
        CHECK(state.apply(bootart::splash::MarkRunning{}) == bootart::splash::TransitionResult::changed);
        return state;
    }

    bootart::splash::PromptMetadata prompt(std::uint64_t request_id) {
        bootart::splash::PromptMetadata result(request_id, "Password for encrypted root:");
        result.with_source("systemd-cryptsetup").with_requester_pid(42).with_expiry(50'000);
        return result;
    }

    void installer_backend_contracts() {
        using namespace bootart::install;
        const auto elf = static_elf_fixture();
        const auto digest = bootart::sha256(std::string_view("known-good"));
        ...
        std::vector<ArchiveEntry> dracut_entries{file("usr/lib/systemd/systemd", 0755, text_bytes("systemd")),
                                                 ...
                                                 file("usr/bin/bootart", 0755, elf)};
        for (const auto id :
             {bootart::embedded::TemplateId::systemd_start_unit, bootart::embedded::TemplateId::systemd_show_unit,
              bootart::embedded::TemplateId::systemd_switch_root_unit,
              bootart::embedded::TemplateId::systemd_console_agent_drop_in}) {
            const auto resource = bootart::embedded::template_resource(id);
        ...
        for (const auto &[path, target] : std::array{
                 std::pair<std::string_view, std::string_view>{
                     "usr/lib/systemd/system/initrd.target.wants/bootart-start.service", "../bootart-start.service"},
                 std::pair<std::string_view, std::string_view>{
                     "usr/lib/systemd/system/initrd.target.wants/bootart-show.service", "../bootart-show.service"},
                 std::pair<std::string_view, std::string_view>{
        ...
        CHECK(dracut_inspection.bootart_digest == bootart::sha256(elf));
        ...
        CHECK(dracut_record.active_digest == bootart::sha256(candidate_bytes));
        CHECK(dracut_systemd_bootart_free_generate_request(dracut_record).arguments[3] == "--omit");
        ...
        std::vector<ArchiveEntry> initramfs_entries{
            file("main/usr/bin/bootart", 0755, elf), file("main/init", 0755, text_bytes("init")),
            file("main/scripts/local-top/cryptroot", 0755, text_bytes("cryptroot")),
            file("main/usr/lib/cryptsetup/askpass.bootart-console", 0755, text_bytes("console")),
            file("main/usr/lib/cryptsetup/functions", 0644, text_bytes("functions"))};
        for (const auto [path, id] : std::array{
                 std::pair<std::string_view, bootart::embedded::TemplateId>{
                     "main/scripts/init-top/bootart", bootart::embedded::TemplateId::initramfs_tools_early_hook},
                 std::pair<std::string_view, bootart::embedded::TemplateId>{
                     "main/scripts/init-bottom/bootart", bootart::embedded::TemplateId::initramfs_tools_bottom_hook},
                 std::pair<std::string_view, bootart::embedded::TemplateId>{
                     "main/usr/lib/cryptsetup/askpass",
                     bootart::embedded::TemplateId::initramfs_tools_askpass_wrapper}}) {
            const auto resource = bootart::embedded::template_resource(id);
        ...
        CHECK(std::string(reinterpret_cast<const char *>(mkinitcpio_contract.config_activated.data()),
                  ...
                  .contains("encrypt bootart filesystems"));
        std::vector<ArchiveEntry> mkinitcpio_entries{file("usr/bin/bootart", 0755, elf),
        ...
        for (const auto [path, id] :
             std::array{std::pair<std::string_view, bootart::embedded::TemplateId>{
                            "hooks/bootart", bootart::embedded::TemplateId::mkinitcpio_runtime_hook},
                        std::pair<std::string_view, bootart::embedded::TemplateId>{
                            "usr/bin/plymouth", bootart::embedded::TemplateId::mkinitcpio_plymouth_bridge}}) {
            const auto resource = bootart::embedded::template_resource(id);
        ...
        const auto path_fact = [](std::string path, std::uint16_t mode, std::string_view contents) {
            return MkinitfsOpenRcPathFact{std::move(path), true, true, false, false, mode, bootart::sha256(contents)};
        };
        ...
        CHECK(text_bytes("features=\"base ext4 virtio bootart\"\n") ==
              activate_mkinitfs_bootart_feature(mkinitfs_config));
        const auto settings =
        ...
        CHECK(parse_extlinux_entry_command_line("LABEL virt\n  APPEND root=LABEL=/ quiet\n", "virt") ==
              "root=LABEL=/ quiet");
        const auto patched_init = bootart::integration::patch_mkinitfs_init(init_source);
        CHECK(patched_init.has_value());
        const auto runtime_bytes = text_bytes(bootart::integration::mkinitfs::runtime_hook);
        const auto findfs_bytes = text_bytes(bootart::integration::mkinitfs::findfs_wrapper);
        const auto patched_bytes = text_bytes(*patched_init);
        const std::array mkinitfs_inputs{
            bootart::integration::CpioInput{"usr/bin/bootart", elf, 0100755},
            bootart::integration::CpioInput{"usr/libexec/bootart/mkinitfs-runtime", runtime_bytes, 0100755},
            bootart::integration::CpioInput{"usr/libexec/bootart/mkinitfs-findfs", findfs_bytes, 0100755},
            bootart::integration::CpioInput{"init", patched_bytes, 0100755}};
        const auto mkinitfs_archive = bootart::integration::build_cpio_archive(mkinitfs_inputs);
        ...
        const std::string pristine_functions = R"BOOTART(prefix
unlock_root_partition() {
	command -v cryptsetup >/dev/null || return
	if cryptsetup isLuks "$PMOS_ROOT"; then
		splash_hide
...
suffix
        ...
        MkinitfsBootDeployFacts deploy_facts{
            ...
            std::string(bootart::integration::reviewed_boot_deploy_initramfs_version),
            pristine_functions,
            "/boot/vmlinuz",
            8 * 1024 * 1024,
            "/boot/initramfs",
        ...
        CHECK(deploy_contract.generate.arguments == std::vector<std::string>({"-d", "/boot/.bootart-candidate"}));
        ...
        CHECK(mkinitfs_boot_deploy_managed_image_path("/boot/.bootart-candidate/boot.img"));
        ...
        const auto patched_deploy = bootart::integration::patch_boot_deploy_init_functions(
            pristine_functions, bootart::integration::reviewed_boot_deploy_initramfs_version);
        CHECK(patched_deploy.has_value());
        const auto deploy_runtime = text_bytes(bootart::integration::mkinitfs_boot_deploy::runtime_hook);
        const auto deploy_fde = text_bytes(bootart::integration::mkinitfs_boot_deploy::fde_wrapper);
        const auto deploy_stock = text_bytes(bootart::integration::mkinitfs_boot_deploy::stock_fde_unlock);
        const auto deploy_unl0kr = text_bytes(bootart::integration::mkinitfs_boot_deploy::native_unl0kr);
        const auto deploy_start = text_bytes(bootart::integration::mkinitfs_boot_deploy::start_hook);
        const auto deploy_cleanup = text_bytes(bootart::integration::mkinitfs_boot_deploy::cleanup_hook);
        const auto patched_deploy_bytes = text_bytes(*patched_deploy);
        const std::array deploy_inputs{
            bootart::integration::CpioInput{".", {}, 0040755},
            bootart::integration::CpioInput{"usr/libexec/bootart", {}, 0040755},
            bootart::integration::CpioInput{"usr/libexec/bootart/native-bin", {}, 0040755},
            bootart::integration::CpioInput{"usr/bin/bootart", elf, 0100755},
            bootart::integration::CpioInput{"usr/libexec/bootart/mkinitfs-boot-deploy-runtime", deploy_runtime,
                                            0100755},
            bootart::integration::CpioInput{"usr/libexec/bootart/mkinitfs-boot-deploy-fde", deploy_fde, 0100755},
            bootart::integration::CpioInput{"usr/libexec/bootart/fde-unlock-stock", deploy_stock, 0100755},
            bootart::integration::CpioInput{"usr/libexec/bootart/native-bin/unl0kr", deploy_unl0kr, 0100755},
            bootart::integration::CpioInput{"hooks-extra/50-bootart-start.sh", deploy_start, 0100755},
            bootart::integration::CpioInput{"hooks-cleanup/90-bootart-handoff.sh", deploy_cleanup, 0100755},
            bootart::integration::CpioInput{"init_functions_2nd.sh", patched_deploy_bytes, 0100644}};
        const auto deploy_archive = bootart::integration::build_cpio_archive(deploy_inputs);
        ...
        const std::array clean_inputs{bootart::integration::CpioInput{"init_functions_2nd.sh", pristine_bytes, 0100644},
                                      bootart::integration::CpioInput{"usr/bin/fde-unlock", shell_bytes, 0100755},
                                      bootart::integration::CpioInput{"usr/bin/unl0kr", shell_bytes, 0100755},
                                      bootart::integration::CpioInput{"usr/sbin/cryptsetup", shell_bytes, 0100755}};
        CHECK(inspect_bootart_free_mkinitfs_boot_deploy_archive(bootart::integration::build_cpio_archive(clean_inputs))
                  .inspected_entries == 4);
        ...
        CHECK(activated_partition_digest == bootart::sha256(std::span(partition).first(original_partition.size())));
    ...
    void check_protocol_error(bootart::splash::ProtocolErrorCode expected, Function &&function) {
        try {
            function();
        } catch (const bootart::splash::ProtocolError &error) {
            CHECK(error.code() == expected);
            return;
        }
    ...
    void runtime_and_client_contracts() {
        using namespace bootart::splash;
        const auto root = std::filesystem::path(BOOTART_SOURCE_ROOT) / "target/cpp";
        ...
        const RuntimePaths paths(runtime);
        {
            ...
            const auto *binary = std::getenv("BOOTART_BINARY");
            CHECK(binary != nullptr);
            const auto command =
                std::string(binary) + " ping --runtime-dir " + runtime.string() + " >/tmp/bootart-cpp-ping.out";
            CHECK(std::system(command.c_str()) == 0);
            CHECK(read_file("/tmp/bootart-cpp-ping.out") == "pong\n");
            cli_server.join();
        }
    ...
    void daemon_process_contracts() {
        ...
        CHECK(binary != nullptr);
        const auto runtime =
            std::filesystem::path(BOOTART_SOURCE_ROOT) / "target/cpp" / ("daemon-test-" + std::to_string(getpid()));
        std::error_code ignored;
        std::filesystem::remove_all(runtime, ignored);
        const auto child = fork();
        CHECK(child >= 0);
    ...
    void binary_smoke() {
        const auto *binary = std::getenv("BOOTART_BINARY");
        CHECK(binary != nullptr);
        const auto version = std::string(binary) + " --version >/tmp/bootart-cpp-version.out";
        CHECK(std::system(version.c_str()) == 0);
        CHECK(read_file("/tmp/bootart-cpp-version.out") == "bootart 0.1.0\n");
        const auto render =
            std::string(binary) + " render-final --no-color --cols 10 --rows 5 >/tmp/bootart-cpp-render.out";
        CHECK(std::system(render.c_str()) == 0);
        const auto output = read_file("/tmp/bootart-cpp-render.out");
