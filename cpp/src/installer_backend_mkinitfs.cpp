...
namespace bootart::install {
    namespace {
        ...
        constexpr std::string_view extlinux_fragment = "/etc/update-extlinux.d/50-bootart-known-good";
        ...
        std::string candidate_image(std::string_view flavor) {
            return "/boot/.bootart-candidate-initramfs-" + std::string(flavor);
        }
        ...
        std::vector<std::byte> deactivate_feature(std::string_view source) {
            ...
            const auto found = std::ranges::find(features, "bootart");
            if (found == features.end())
                throw std::runtime_error("mkinitfs configuration omits bootart");
            features.erase(found);
            if (features.empty())
                throw std::runtime_error("mkinitfs configuration contains only bootart");
        ...
        std::vector<std::byte> render_extlinux(std::string_view kernel_path, std::string_view known_good_path,
            ...
            const std::string output = "LABEL bootart-known-good\n"
                                       ...
                                       "\n"
                                       "  APPEND " +
                                       std::string(command_line) + " bootart=0 rd.bootart=0\n";
            return bytes(output);
        }
    ...
    std::vector<std::byte> activate_mkinitfs_bootart_feature(std::string_view source) {
        auto features = parse_mkinitfs_features(source);
        if (std::ranges::find(features, "bootart") != features.end()) {
            throw std::runtime_error("mkinitfs configuration contains unmanaged bootart");
        }
        features.emplace_back("bootart");
        return render_features(source, features);
    }
    ...
    MkinitfsOpenRcContract plan_mkinitfs_openrc(const MkinitfsOpenRcFacts &facts, std::string alternate_root) {
        ...
        const bool already = std::ranges::find(facts.mkinitfs_features, "bootart") != facts.mkinitfs_features.end();
        const auto original =
    ...
    void validate_mkinitfs_openrc_contract(const MkinitfsOpenRcContract &contract) {
        ...
        if (config == contract.prerequisites.end() || contract.mkinitfs_config_original.empty() ||
            ...
            activate_mkinitfs_bootart_feature(text(contract.mkinitfs_config_original)) !=
    ...
    ArchiveInspection inspect_mkinitfs_openrc_archive(std::span<const std::byte> candidate,
        ...
        std::optional<std::vector<std::byte>> bootart, runtime, findfs, init;
        bool trailer = false;
        while (offset < candidate.size()) {
            ...
            if (path->contains("bootart") && *path != "usr/bin/bootart" && *path != "usr/libexec/bootart" &&
                *path != "usr/libexec/bootart/mkinitfs-runtime" && *path != "usr/libexec/bootart/mkinitfs-findfs") {
                throw std::runtime_error("foreign Bootart mkinitfs member");
            }
            ...
            if (*path == "usr/bin/bootart")
                capture(bootart, true);
            else if (*path == "usr/libexec/bootart/mkinitfs-runtime")
                capture(runtime, true);
            else if (*path == "usr/libexec/bootart/mkinitfs-findfs")
                capture(findfs, true);
            else if (*path == "init")
                capture(init, false);
            offset = align4(data_start + data.size());
        }
        if (!trailer || !bootart || !runtime || !findfs || !init ||
            *bootart != std::vector<std::byte>(expected_bootart.begin(), expected_bootart.end()) ||
        ...
        return {sha256(expected_bootart), entries, inspected};
    }
    ...
    bool mkinitfs_openrc_managed_image_path(std::string_view path) {
        ...
        constexpr std::string_view candidate = "/boot/.bootart-candidate-initramfs-";
        ...
        if (flavor.ends_with(".bootart-known-good"))
            flavor.remove_suffix(std::string_view(".bootart-known-good").size());
