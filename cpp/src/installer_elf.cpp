...
namespace bootart::install {
    namespace {
        ...
        std::uint16_t expected_machine() {
            ...
            return 183;
#else
#error "Bootart installer supports x86_64 and aarch64"
#endif
        }
    ...
    void validate_static_elf(std::span<const std::byte> bytes) {
        if (bytes.size() > max_install_file_bytes) {
            throw std::runtime_error("Bootart ELF exceeds the installer size limit");
        }
        ...
        if (read_little<std::uint16_t>(bytes, 18) != expected_machine()) {
            throw std::runtime_error("payload machine does not match Bootart");
        }
    ...
    std::vector<std::byte> read_running_elf() {
        ...
        if (fstat(descriptor, &status) != 0 || !S_ISREG(status.st_mode) || status.st_size < 0) {
            throw std::runtime_error("running Bootart descriptor is not a regular file");
        }
        if (static_cast<std::uint64_t>(status.st_size) > max_install_file_bytes) {
            throw std::runtime_error("running Bootart ELF exceeds the installer size limit");
        }
        ...
        while (offset < bytes.size()) {
            ...
            if (count <= 0) {
                throw std::runtime_error("running Bootart ELF changed while being read");
            }
            offset += static_cast<std::size_t>(count);
        }
        std::byte extra{};
        if (read(descriptor, &extra, 1) != 0) {
            throw std::runtime_error("running Bootart ELF changed size while being read");
        }
