...
namespace bootart::splash {
        ...
        void compare_executables(int candidate, int running, off_t size) {
            ...
            while (offset < size) {
                ...
                if (!std::equal(left.begin(), left.begin() + left_count, right.begin())) {
                    throw std::runtime_error("new-root bootart differs from the running executable");
                }
                offset += left_count;
            }
        }
    ...
    void LinuxSelfRootTransition::transition(const std::filesystem::path &new_root) {
        ...
        const auto candidate = secure_open(bin.get(), "bootart", O_RDONLY | O_CLOEXEC | O_NOFOLLOW,
                                           RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS | RESOLVE_NO_MAGICLINKS);
        const auto candidate_before = inspect_root_object(candidate.get(), false, "candidate bootart");
        const OwnedDescriptor running(open("/proc/self/exe", O_RDONLY | O_CLOEXEC));
        if (running.get() < 0)
            root_error("open running executable");
        const auto running_before = inspect_root_object(running.get(), false, "running bootart");
        if (candidate_before.size != running_before.size) {
            throw std::runtime_error("new-root bootart size differs from the running executable");
        }
        ...
        if (inspect_root_object(root.get(), true, "candidate root") != root_before ||
            ...
            inspect_root_object(candidate.get(), false, "candidate bootart") != candidate_before ||
            inspect_root_object(running.get(), false, "running bootart") != running_before) {
