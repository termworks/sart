...
namespace bootart::install {
    namespace {
        ...
        std::string candidate_image(std::string_view package) {
            return "/boot/.bootart-candidate-initramfs-" + std::string(package) + ".img";
        }

        std::optional<std::string_view> candidate_package(std::string_view path) {
            constexpr std::string_view prefix = "/boot/.bootart-candidate-initramfs-";
    ...
    std::expected<std::string, std::string> activate_mkinitcpio_hooks(std::string_view source) {
        ...
        if (const auto bootart = position("bootart")) {
            if (*bootart != *encrypt + 1)
                return std::unexpected("mkinitcpio bootart hook is in an unsafe position");
            return std::string(source);
        }
        hooks.insert(hooks.begin() + static_cast<std::ptrdiff_t>(*encrypt + 1), "bootart");
    ...
    GeneratorRequest mkinitcpio_unpack_request(const MkinitcpioSystemdContract &contract,
        ...
        GeneratorRequest request{GeneratorKind::initramfs_inspection,
                                 ...
                                 "/var/lib/bootart/install/transactions/" + std::string(transaction) +
                                     "/unpacked-candidate",
    ...
    ArchiveInspection inspect_mkinitcpio_inventory(const std::vector<ArchiveEntry> &entries,
        ...
        const auto &bootart = require("usr/bin/bootart");
        if (bootart.kind != ArchiveEntryKind::file || bootart.mode != 0755 ||
            bootart.bytes != std::vector<std::byte>(expected_bootart.begin(), expected_bootart.end())) {
            throw std::runtime_error("mkinitcpio contains the wrong Bootart ELF");
        }
        for (const auto [path, id] :
             std::array{std::pair<std::string_view, embedded::TemplateId>{
                            "hooks/bootart", embedded::TemplateId::mkinitcpio_runtime_hook},
        ...
        for (const auto &[path, entry] : seen) {
            ...
            if (name.starts_with("bootart") && path != "usr/bin/bootart" && path != "hooks/bootart") {
                throw std::runtime_error("unexpected Bootart mkinitcpio member: " + std::string(path));
            }
        }
        return {sha256(expected_bootart), entries.size(), inspected};
    }
