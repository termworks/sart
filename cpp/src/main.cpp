...
#include "bootart/splash/daemon.hpp"
...
namespace {
    ...
    std::filesystem::path extract_runtime_directory(std::vector<std::string_view> &arguments) {
        auto runtime = std::filesystem::path(bootart::splash::default_runtime_directory);
    ...
    bootart::splash::Mode parse_mode(std::string_view value) {
        using bootart::splash::Mode;
        if (value == "boot")
            return Mode::boot;
        if (value == "shutdown")
    ...
    bootart::password::NativeAdapter parse_native_adapter(std::string_view value) {
        using bootart::password::NativeAdapter;
    ...
    int run_control(std::filesystem::path runtime, const bootart::splash::Frame &request) {
        const bootart::splash::ClientConfig config(bootart::splash::RuntimePaths(std::move(runtime)));
        const auto response = bootart::splash::send_request(config, request);
        using bootart::splash::Opcode;
    ...
    int run_daemon_command(std::vector<std::string_view> arguments) {
        ...
        auto mode = bootart::splash::Mode::boot;
        bool test_buffer{};
        auto cmdline = std::filesystem::path(bootart::cmdline::proc_cmdline);
        auto display = bootart::TextVtConfig::open_query();
        auto engine = bootart::splash::EngineConfig{};
        auto password_broker = bootart::splash::PasswordBroker::none;
        for (std::size_t index = 0; index < arguments.size(); ++index) {
            if (arguments[index] == "--test-buffer") {
                ...
                display = bootart::TextVtConfig::configured(number);
                ...
                if (broker == "none")
                    password_broker = bootart::splash::PasswordBroker::none;
                else if (broker == "systemd")
                    password_broker = bootart::splash::PasswordBroker::systemd;
                else if (broker == "native")
                    password_broker = bootart::splash::PasswordBroker::native;
                else
        ...
        if (test_buffer && display.selection().kind == bootart::VtSelectionKind::configured) {
            usage_error("--test-buffer conflicts with --tty");
        }
        engine.validate();
        bootart::signals::reset_stop_flag();
        bootart::signals::SignalGuard signal_guard;
        bootart::splash::DaemonConfig config{bootart::splash::RuntimePaths(std::move(runtime)), mode, test_buffer};
        config.cmdline = std::move(cmdline);
        config.display = display;
        config.engine = engine;
        config.password_broker = password_broker;
        bootart::splash::run_daemon(config);
        return 0;
    }
    ...
    [[noreturn]] void usage_error(std::string_view message) {
        std::cerr << "bootart: " << message << '\n';
        std::cerr << "Try 'bootart --help' for more information.\n";
        std::exit(2);
    }
    ...
    std::string read_asset(const std::optional<std::filesystem::path> &path) {
        if (!path) {
            return std::string(bootart::embedded::default_art);
        }
        ...
        while (input) {
            ...
            if (result.size() > bootart::max_art_bytes) {
...
Commands:
  ...
  uninstall  Restore installer preimages
    ...
    void print_install_action_help(std::string_view action) {
        if (action == "plan") {
            std::cout << "Usage: bootart install plan [--json]\n";
        } else if (action == "status") {
            std::cout << "Usage: bootart install status\n";
        } else if (action == "apply" || action == "recover" || action == "uninstall") {
            std::cout << "Usage: bootart install " << action << " --confirm-host HOSTNAME\n";
        } else {
    ...
    int run_install_command(std::vector<std::string_view> arguments) {
        ...
        if (action == "status") {
            ...
            std::cout << bootart::install::render_status(bootart::install::Installer::live_root_read_only().status());
            return 0;
        }
        if (action == "plan") {
            bool json = false;
            ...
            const auto discovery = bootart::install::discover_exact_install_contract();
            const auto plan = bootart::install::build_exact_self_install_plan(discovery);
            std::cout << bootart::install::render_exact_install_plan(plan, discovery, json);
            return 0;
        }
        if (action == "apply" || action == "recover" || action == "uninstall") {
            ...
            auto installer = bootart::install::Installer::live_root_mutating(*confirmation, package_hook);
            if (action == "recover") {
                const auto outcome = installer.recover();
                std::cout << "bootart install recover: "
                          << (outcome == bootart::install::RecoveryOutcome::no_transaction ? "nothing-to-recover"
                              : outcome == bootart::install::RecoveryOutcome::rolled_back  ? "rolled-back"
            ...
            if (action == "apply") {
                ...
                std::cout << "bootart install apply: "
                          << (outcome == bootart::install::ApplyOutcome::installed   ? "installed"
                              : outcome == bootart::install::ApplyOutcome::refreshed ? "refreshed"
                                                                                     : "already-current")
                          << '\n';
                return 0;
            }
            const auto report = installer.uninstall(&discovery);
            std::cout << "bootart install uninstall: removed=" << report.removed << " restored=" << report.restored
    ...
    int run_visual(std::string_view command, const std::vector<std::string_view> &arguments) {
        ...
        const auto small =
            options.asset ? std::optional<Art>{} : std::optional<Art>{Art::parse(bootart::embedded::small_art)};
        bootart::StdoutTerminal terminal(options.columns, options.rows);
        bootart::signals::SignalGuard signal_guard;
        if (final) {
            bootart::render_final(terminal, art, small ? &*small : nullptr, options.no_color);
            return 0;
        }
        bootart::RenderOptions render_options{
        ...
        do {
            bootart::play_animation(terminal, art, small ? &*small : nullptr, render_options, iteration++);
        } while (preview && options.loop && !bootart::signals::should_stop());
        return 0;
    }
    ...
    int run(int argc, char **argv) {
        ...
        if (command == "early-boot-enabled") {
            std::filesystem::path path(bootart::cmdline::proc_cmdline);
            if (!arguments.empty()) {
            ...
            return bootart::cmdline::early_boot_enabled_at(path) ? 0 : 1;
        }
        if (command == "console-fallback-needed") {
            ...
            do {
                try {
                    auto config = bootart::splash::ClientConfig(bootart::splash::RuntimePaths(runtime));
                    config.timeout = std::chrono::milliseconds(100);
                    const auto response = bootart::splash::send_request(
                        config, bootart::splash::Frame::empty(bootart::splash::Opcode::ping,
                                                              bootart::splash::next_request_id()));
                    if (response.opcode() == bootart::splash::Opcode::pong)
                        return 1;
                } catch (...) {
                }
                if (std::chrono::steady_clock::now() >= deadline)
        ...
        if (command == "native-ready") {
            ...
            try {
                const auto response =
                    bootart::splash::send_request(bootart::splash::ClientConfig(bootart::splash::RuntimePaths(runtime)),
                                                  bootart::splash::Frame::empty(bootart::splash::Opcode::native_ready,
                                                                                bootart::splash::next_request_id()));
                return response.opcode() == bootart::splash::Opcode::ack ? 0 : 75;
            } catch (...) {
                return 75;
            }
        }
        if (command == "native-askpass") {
            std::optional<bootart::password::NativeAdapter> adapter;
            ...
            try {
                const auto outcome =
                    bootart::password::run_native_askpass_client(*adapter, {*prompt, attempts, maximum_secret_bytes},
                                                                 bootart::password::claim_native_askpass_output());
                if (outcome == bootart::password::NativeAskpassOutcome::delivered)
                    return 0;
                if (outcome == bootart::password::NativeAskpassOutcome::user_cancelled) {
                    return bootart::password::native_askpass_cancelled_exit_code;
                }
                return bootart::password::native_askpass_transport_exit_code;
            } catch (...) {
                return bootart::password::native_askpass_transport_exit_code;
            }
        }
...
int main(int argc, char **argv) {
    if (!bootart::process_is_allowed(static_cast<std::uint32_t>(getpid()))) {
        std::cerr << "bootart refuses to run as PID 1\n";
        return bootart::pid1_refusal_exit_code;
    }
    try {
        return run(argc, argv);
    } catch (const std::exception &error) {
        std::cerr << "bootart: " << error.what() << '\n';
        return 1;
    }
}
