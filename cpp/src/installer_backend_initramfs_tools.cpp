...
namespace bootart::install {
    namespace {
        ...
        std::optional<std::string_view> candidate_kernel(std::string_view path) {
            constexpr std::string_view prefix = "/boot/.bootart-candidate-initrd.img-";
            if (!path.starts_with(prefix))
                return std::nullopt;
            const auto kernel = path.substr(prefix.size());
            return safe_token(kernel, 128) ? std::optional(kernel) : std::nullopt;
        ...
        std::map<std::string, std::pair<std::uint16_t, std::vector<std::byte>>> expected_files() {
            ...
            for (const auto [path, id] : std::array{
                     std::pair<std::string_view, embedded::TemplateId>{
                         "main/scripts/init-top/bootart", embedded::TemplateId::initramfs_tools_early_hook},
                     std::pair<std::string_view, embedded::TemplateId>{
                         "main/scripts/init-bottom/bootart", embedded::TemplateId::initramfs_tools_bottom_hook},
                     std::pair<std::string_view, embedded::TemplateId>{
        ...
        bool bootart_namespace(std::string_view path) {
            const auto slash = path.rfind('/');
            const auto name = slash == std::string_view::npos ? path : path.substr(slash + 1);
            if (name == "bootart" || name.starts_with("askpass.bootart") || path.starts_with("main/usr/lib/bootart/")) {
                return true;
            }
            for (const auto prefix : {"main/bin/", "main/sbin/", "main/usr/bin/", "main/usr/sbin/"}) {
                if (path.starts_with(prefix) && name.starts_with("bootart"))
                    return true;
            }
            return false;
        }
    ...
    void validate_initramfs_tools_systemd_generator_request(const GeneratorRequest &value) {
        ...
        if (value.generator == GeneratorKind::initramfs_inspection && value.executable == unmkinitramfs) {
            constexpr std::string_view prefix = "/var/lib/bootart/install/transactions/";
    ...
    GeneratorRequest initramfs_tools_systemd_unpack_request(const InitramfsToolsSystemdContract &contract,
        ...
        auto value = request(GeneratorKind::initramfs_inspection, unmkinitramfs, contract.generate.alternate_root,
                             {contract.candidate_image, "/var/lib/bootart/install/transactions/" +
    ...
    ArchiveInspection inspect_initramfs_tools_inventory(const std::vector<ArchiveEntry> &entries,
        ...
        const auto bootart = seen.find("main/usr/bin/bootart");
        if (bootart == seen.end() || bootart->second->kind != ArchiveEntryKind::file || bootart->second->mode != 0755 ||
            ...
            throw std::runtime_error("initramfs-tools contains the wrong Bootart ELF");
        }
        ...
        for (const auto path :
             {"main/init", "main/scripts/local-top/cryptroot", "main/usr/lib/cryptsetup/askpass.bootart-console"})
        ...
        for (const auto &[path, entry] : seen) {
            if (!bootart_namespace(path))
                continue;
            const bool allowed = path == "main/usr/bin/bootart" || expected.contains(std::string(path)) ||
                                 path == "main/usr/lib/cryptsetup/askpass.bootart-console";
            const bool executable_allowed = path == "main/usr/bin/bootart" || path == "main/scripts/init-top/bootart" ||
                                            path == "main/scripts/init-bottom/bootart" ||
            ...
            if (!allowed || path.contains("bootart-init") ||
                ...
                throw std::runtime_error("unexpected Bootart initramfs-tools member: " + std::string(path));
            }
        }
    ...
    inspect_bootart_free_initramfs_tools_inventory(const std::vector<ArchiveEntry> &entries) {
        const auto [seen, total] = common_inventory(entries);
        if (std::ranges::any_of(seen, [](const auto &item) { return bootart_namespace(item.first); })) {
            throw std::runtime_error("Bootart-free initramfs-tools archive contains a Bootart member");
        }
        ...
        if (functions == seen.end() || functions->second->kind != ArchiveEntryKind::file ||
            functions->second->bytes.empty()) {
            throw std::runtime_error("Bootart-free initramfs-tools functions are missing");
        }
    ...
    DracutSystemdImageRecord verified_initramfs_tools_systemd_image_record(
        ...
        const ArchiveInspection &inspection, std::span<const std::byte> expected_bootart) {
        validate_initramfs_tools_systemd_contract(contract);
