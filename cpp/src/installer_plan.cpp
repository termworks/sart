...
namespace bootart::install {
    namespace {
        ...
        std::string source_name(const PlanSource &source) {
            if (source.kind == PlanSourceKind::bootart_elf) {
                return "bootart.elf";
            }
    ...
    std::string InstallPlan::identity() const {
        ...
        push_field(bytes, "bootart.install-plan.identity");
    ...
    InstallPlan build_install_plan(std::span<const std::byte> bootart_elf, AdapterId initramfs, AdapterId real_root,
        ...
        plan.operations.push_back({
            ...
            0,
            sha256(bootart_elf),
            {PlanSourceKind::bootart_elf, {}},
            {bootart_elf.begin(), bootart_elf.end()},
        });
        std::set<std::string> paths{"/usr/bin/bootart"};
        ...
        if (real_root == AdapterId::systemd_real_root) {
            ...
            add_activation(plan.activations,
                           {real_root, ActivationScope::real_root, ActivationRelation::openrc_runlevel,
                            "/etc/runlevels/boot/bootart", "../../init.d/bootart", 0,
                            embedded::TemplateId::openrc_supervisor_script, "boot"});
            add_activation(plan.activations,
                           {real_root, ActivationScope::real_root, ActivationRelation::openrc_runlevel,
                            "/etc/runlevels/default/bootart-quit", "../../init.d/bootart-quit", 0,
    ...
    std::string render_plan_human(const InstallPlan &plan, bool actionable) {
        std::string output = std::format("bootart install plan v{}\nstatus: {}\nmutation: {}\nresource-set: "
                                         "{}\nplan-id: {}\nroot: {}\nadapters: {} + {}\noperations:\n",
    ...
    std::string render_plan_json(const InstallPlan &plan, bool actionable) {
        ...
        return std::format("{{\"schema\":\"bootart.install-plan\",\"version\":{},\"resource_set_version\":{},\"plan_"
