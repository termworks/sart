...
namespace bootart::install {
    namespace {
        ...
        std::string candidate_image(DracutImageLayout layout, std::string_view kernel) {
            if (layout == DracutImageLayout::initrd_img)
                return "/boot/.bootart-candidate-initrd.img-" + std::string(kernel);
            return "/boot/.bootart-candidate-initramfs-" + std::string(kernel) + ".img";
        }
        ...
        std::optional<std::pair<DracutImageLayout, std::string_view>> candidate_parts(std::string_view path) {
            constexpr std::string_view initrd = "/boot/.bootart-candidate-initrd.img-";
            constexpr std::string_view initramfs = "/boot/.bootart-candidate-initramfs-";
            if (path.starts_with(initrd)) {
                const auto kernel = path.substr(initrd.size());
                if (safe_token(kernel, 128))
                    return std::pair{DracutImageLayout::initrd_img, kernel};
        ...
        bool starts_with_bootart_namespace(std::string_view path) { return path.contains("bootart"); }
    ...
    DracutSystemdContract plan_dracut_systemd(const DracutSystemdFacts &facts, std::string alternate_root) {
        ...
        const auto known_good = active + ".bootart-known-good";
        DracutSystemdContract contract{
            ...
            "/etc/grub.d/41_bootart_known_good",
            ...
            request(GeneratorKind::dracut, dracut, alternate_root,
                    {"--force", "--kver", kernel, "--add", "bootart-systemd", candidate}),
    ...
    void validate_dracut_systemd_generator_request(const GeneratorRequest &value) {
        ...
        if (value.generator == GeneratorKind::dracut && value.executable == dracut) {
            if (value.working_directory || value.arguments.size() != 6 || value.arguments[0] != "--force" ||
                ...
                value.arguments[4] != "bootart-systemd") {
        ...
        if (value.generator == GeneratorKind::initramfs_inspection && value.executable == lsinitrd) {
            if (value.arguments.size() != 2 || value.arguments[0] != "--unpack" ||
                ...
                !value.working_directory->starts_with("/var/lib/bootart/install/transactions/") ||
            ...
            const auto transaction = std::string_view(*value.working_directory)
                                         .substr(std::string_view("/var/lib/bootart/install/transactions/").size(),
                                                 value.working_directory->size() -
                                                     std::string_view("/var/lib/bootart/install/transactions/").size() -
    ...
    void validate_dracut_systemd_contract(const DracutSystemdContract &contract) {
        if (!safe_token(contract.kernel_version, 128) ||
            ...
            contract.known_good_image != contract.active_image + ".bootart-known-good" ||
    ...
    GeneratorRequest dracut_systemd_unpack_request(const DracutSystemdContract &contract,
        ...
        value.working_directory =
            "/var/lib/bootart/install/transactions/" + std::string(transaction) + "/unpacked-candidate";
        validate_dracut_systemd_generator_request(value);
        return value;
    }

    bool dracut_systemd_managed_image_path(std::string_view path) {
        if (path == "/etc/grub.d/41_bootart_known_good" || path == "/boot/grub/grub.cfg" ||
            path == "/boot/grub2/grub.cfg" || candidate_parts(path))
            return true;
        if (path.ends_with(".bootart-known-good"))
            path.remove_suffix(std::string_view(".bootart-known-good").size());
    ...
    ArchiveInspection inspect_dracut_inventory(const std::vector<ArchiveEntry> &entries,
        ...
        const auto bootart = seen.find("usr/bin/bootart");
        if (bootart == seen.end() || bootart->second->kind != ArchiveEntryKind::file || bootart->second->mode != 0755 ||
            bootart->second->bytes != std::vector<std::byte>(expected_bootart.begin(), expected_bootart.end())) {
            throw std::runtime_error("dracut contains the wrong Bootart ELF");
        }
        validate_static_elf(bootart->second->bytes);
        ...
        for (const auto &[path, entry] : seen) {
            if (!starts_with_bootart_namespace(path))
                continue;
            const bool allowed =
                path == "usr/bin/bootart" || expected.contains(std::string(path)) ||
            ...
            if (!allowed || path.contains("bootart-init") ||
                (entry->kind == ArchiveEntryKind::file && (entry->mode & 0111) != 0 && path != "usr/bin/bootart")) {
                throw std::runtime_error("unexpected Bootart dracut member: " + std::string(path));
            }
        }
        return {sha256(expected_bootart), entries.size(), total};
    }

    BootartFreeArchiveInspection inspect_bootart_free_dracut_inventory(const std::vector<ArchiveEntry> &entries) {
        const auto [seen, total] = common_inventory(entries);
        if (std::ranges::any_of(seen, [](const auto &item) { return item.first.contains("bootart"); })) {
            throw std::runtime_error("Bootart-free dracut archive contains a Bootart member");
        }
    ...
    void validate_dracut_systemd_image_record(const DracutSystemdImageRecord &record) {
        ...
        if (!active || !candidate || !safe_token(record.kernel_version, 128) || active->first != candidate->first ||
            ...
            record.known_good_image != record.active_image + ".bootart-known-good" ||
    ...
    GeneratorRequest dracut_systemd_bootart_free_generate_request(const DracutSystemdImageRecord &record,
        ...
        auto value =
            request(GeneratorKind::dracut, dracut, std::move(alternate_root),
                    {"--force", "--kver", record.kernel_version, "--omit", "bootart-systemd", record.candidate_image});
        validate_dracut_systemd_generator_request(value);
        return value;
    }

    GeneratorRequest dracut_systemd_bootart_free_unpack_request(const DracutSystemdImageRecord &record,
                                                                std::string_view transaction,
                                                                std::string alternate_root) {
        validate_dracut_systemd_image_record(record);
