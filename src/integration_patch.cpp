...
namespace sart::integration {
    namespace {
        ...
        constexpr std::string_view stock_unlock_function = R"SART(unlock_root_partition() {
    ...
    std::expected<std::string, PatchError> patch_mkinitfs_init(std::string_view input) {
        ...
        if (early_count == 0 && handoff_count == 0 && !input.contains("# sart:begin mkinitfs-") &&
            !input.contains("# sart:end mkinitfs-")) {
    ...
    std::expected<std::string, PatchError> patch_boot_deploy_init_functions(std::string_view input,
        ...
        constexpr std::string_view begin = "# sart:begin mkinitfs-boot-deploy-fde-v1";
        constexpr std::string_view end = "# sart:end mkinitfs-boot-deploy-fde-v1";
