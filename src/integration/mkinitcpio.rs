//! Embedded mkinitcpio build/runtime hooks and guarded Plymouth-compatible
//! password bridge.
//!
//! The reviewed BusyBox `encrypt` hook invokes a `plymouth` command only as a
//! password producer/command bridge.  Bootart supplies that image-local name
//! without shipping another binary: the embedded shell bridge obtains one
//! secret at a time from the native broker and feeds the framework-provided
//! `cryptsetup --key-file=-` command through an anonymous pipe.  If the exact
//! upstream hook shape drifts, the build hook omits the bridge and the runtime
//! hook leaves the stock console prompt visible.
...
run_earlyhook() {
    ...
    (
        ...
        bootart_daemon_ret=$?
        # Normal non-acquisition/failure paths prove that the stock prompt can
        # safely proceed. Exit 77 and signal-style/unknown exits retain the
        # guard because display restoration was not proven.
        case "$bootart_daemon_ret" in
            0 | 1) rm -f -- "$bootart_guard" ;;
        esac
    ) &

    # mkinitcpio invokes the encrypt hook immediately after early hooks. Bound
    # that handoff so its one Plymouth capability probe cannot outrun creation
    # of the native listener. A normal daemon failure removes the guard and
    # releases the reviewed stock path without waiting for the full bound.
    if [ "$bootart_native" = yes ]; then
...
/// Image-local implementation of the two Plymouth client operations used by
/// mkinitcpio's reviewed BusyBox encrypt hook.  It is a shell resource, not a
/// helper executable, and is copied only into a generated initramfs.
pub const PLYMOUTH_BRIDGE: &str = r#"#!/usr/bin/ash
...
# The stock hook appends only an optional literal stdout redirection. Remove
# it before applying the fixed safe-character check, then restore it.
bootart_quiet=
case "$bootart_command" in
    *' >/dev/null')
...
# Restore the VT before allowing the original console behavior. Removing the
# key-file option makes cryptsetup read from the explicitly attached console.
...
mod tests {
    ...
    fn install_hook_uses_mkinitcpio_apis() {
        ...
        assert!(INSTALL_HOOK.contains("add_file \"$bootart_bridge\" /usr/bin/plymouth 755"));
        assert!(INSTALL_HOOK.contains("mkinitcpio encrypt hook changed"));
    ...
    fn runtime_hook_starts_a_child_and_hands_off() {
        ...
        assert!(RUNTIME_HOOK.contains("bootart:mkinitcpio-plymouth-native-v1"));
        assert!(RUNTIME_HOOK.contains(".bootart-mkinitcpio-starting"));
        assert!(RUNTIME_HOOK.contains("sleep 1"));
        assert!(RUNTIME_HOOK.contains("[ ! -e \"$bootart_guard\" ]"));
    ...
    #[test]
    fn plymouth_bridge_is_narrow_and_preserves_framework_unlock() {
        assert!(PLYMOUTH_BRIDGE.contains("--ping)"));
        assert!(PLYMOUTH_BRIDGE.contains("ask-for-password)"));
        assert!(PLYMOUTH_BRIDGE.contains("native-askpass"));
        assert!(PLYMOUTH_BRIDGE.contains("--adapter mkinitcpio-busybox"));
        assert!(PLYMOUTH_BRIDGE.contains("8>&1"));
        assert!(PLYMOUTH_BRIDGE.contains("| eval \"$bootart_command\""));
        assert!(PLYMOUTH_BRIDGE.contains("/usr/bin/bootart quit"));
        assert!(PLYMOUTH_BRIDGE.contains("</dev/console"));
    }
