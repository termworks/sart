...
namespace sart::install {
    namespace {
        ...
        constexpr std::string_view candidate_directory = "/boot/.sart-candidate";
        constexpr std::string_view candidate_image = "/boot/.sart-candidate/initramfs";
        constexpr std::string_view candidate_boot_image = "/boot/.sart-candidate/boot.img";
        constexpr std::string_view known_good_image = "/boot/initramfs.sart-known-good";
        constexpr std::string_view known_good_entry = "/boot/loader/entries/sart-known-good.conf";
        ...
        std::vector<std::byte> render_known_good(std::string_view kernel_image, std::string_view command_line) {
            ...
            const std::string output = "title Sart known-good\nlinux " + std::string(kernel_image.substr(6)) +
                                       "\ninitrd initramfs.sart-known-good\noptions " + std::string(command_line) +
                                       " sart=0 rd.sart=0\n";
            return bytes(output);
        }
    ...
    void validate_mkinitfs_boot_deploy_contract(const MkinitfsBootDeployContract &contract) {
        ...
        if (contract.active_image != active_image || contract.kernel_bytes == 0 ||
            ...
            !text(contract.known_good_entry).ends_with(" sart=0 rd.sart=0\n") ||
