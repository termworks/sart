...
namespace sart::install {
    ...
    ArchiveInspection inspect_mkinitfs_boot_deploy_archive(std::span<const std::byte> candidate,
        ...
        const std::map<std::string, std::pair<std::vector<std::byte>, std::uint16_t>> expected{
            ...
            {"usr/libexec/sart/fde-unlock-stock", {bytes(integration::mkinitfs_boot_deploy::stock_fde_unlock), 0755}},
            {"usr/libexec/sart/native-bin/unl0kr", {bytes(integration::mkinitfs_boot_deploy::native_unl0kr), 0755}},
            {"hooks-extra/50-sart-start.sh", {bytes(integration::mkinitfs_boot_deploy::start_hook), 0755}},
            {"hooks-cleanup/90-sart-handoff.sh", {bytes(integration::mkinitfs_boot_deploy::cleanup_hook), 0755}}};
        const std::set<std::string_view> expected_directories{"usr/libexec/sart", "usr/libexec/sart/native-bin"};
        std::set<std::string> found;
        std::optional<std::vector<std::byte>> sart, init_functions;
        for (const auto &member : members) {
            const auto type = member.mode & 0170000;
            if (member.path.contains("sart") && member.path != "usr/bin/sart" && !expected.contains(member.path) &&
                !expected_directories.contains(member.path)) {
                throw std::runtime_error("foreign Sart boot-deploy member");
            }
            if (member.path == ".") {
            ...
            } else if (member.path == "usr/bin/sart") {
                if (type != 0100000 || member.uid != 0 || (member.mode & 07777) != 0755) {
                    throw std::runtime_error("unsafe boot-deploy Sart metadata");
                }
                sart = member.bytes;
        ...
        if (found.size() != expected.size() || !sart ||
            *sart != std::vector<std::byte>(expected_sart.begin(), expected_sart.end()) || !init_functions) {
        ...
        return {sha256(expected_sart), members.size(), inspected};
    }
    ...
    SartFreeArchiveInspection inspect_sart_free_mkinitfs_boot_deploy_archive(std::span<const std::byte> candidate) {
        ...
        for (const auto &member : members) {
            if (member.path.contains("sart"))
                throw std::runtime_error("Sart-free archive has Sart residue");
            const auto type = member.mode & 0170000;
            ...
            if (member.path == "init_functions_2nd.sh") {
                if (type != 0100000 || member.uid != 0 || (member.mode & 0022) != 0) {
                    throw std::runtime_error("unsafe Sart-free init functions");
                }
                init_functions = member.bytes;
        ...
        if (!init_functions ||
            ...
            throw std::runtime_error("Sart-free archive lacks the stock unlock chain");
        }
