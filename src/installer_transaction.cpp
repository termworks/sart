...
#include "sart/installer_live.hpp"
...
namespace sart::install {
    namespace {
        ...
        constexpr std::string_view manifest_path = "/var/lib/sart/install/manifest.v1";
        constexpr std::string_view journal_path = "/.sart-installer-journal.v1";
        constexpr std::string_view transactions_path = "/var/lib/sart/install/transactions";
        ...
        std::string temporary_name() { return std::format(".sart-tmp-{}-{}", getpid(), unique_counter.fetch_add(1)); }
        ...
        std::string serialize_image(const Manifest &manifest) {
            ...
            return std::format("image\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n",
                               ...
                               hex(image.grub_config_path), image.sart_digest);
        }
        ...
        DracutSystemdImageRecord parse_image(const std::vector<std::string_view> &item, AdapterId initramfs,
            ...
            for (const auto &digest : {image.active_digest, image.candidate_digest, image.known_good_digest,
                                       image.grub_script_digest, image.sart_digest}) {
        ...
        std::string serialize_manifest(const Manifest &manifest) {
            std::string output = "SART-MANIFEST\t3\n";
        ...
        std::string serialize_journal(const Journal &journal) {
            std::string output = "SART-JOURNAL\t1\n";
        ...
        void create_directories(std::string_view root, const std::vector<std::string> &directories,
            ...
            for (const auto &guest : directories) {
                const auto path = host_path(root, guest);
                const auto mode =
                    guest.starts_with("/var/lib/sart/install") || guest.starts_with("/boot/.sart-candidate") ? 0700
                                                                                                             : 0755;
        ...
        void prune_boot_deploy_candidate(std::string_view root, std::string_view candidate_directory,
            ...
            if (candidate_directory != "/boot/.sart-candidate")
        ...
        std::vector<std::string> required_missing_directories(std::string_view root, const std::vector<Entry> &entries,
            ...
            for (const auto state :
                 {std::string_view("/var/lib/sart"), std::string_view("/var/lib/sart/install"), transactions_path}) {
        ...
        bool hook_argument(std::span<const std::byte> argument) {
            constexpr std::string_view absolute = "/etc/apk/commit_hooks.d/95-sart-raw-boot";
            constexpr std::string_view relative = "etc/apk/commit_hooks.d/95-sart-raw-boot";
        ...
        void authorize_package_hook() {
            constexpr std::string_view hook_path = "/etc/apk/commit_hooks.d/95-sart-raw-boot";
            ...
            const auto installed = read_authorization_file("/usr/bin/sart", 0755, max_install_file_bytes);
            const auto running = read_running_elf();
            if (sha256(installed) != sha256(running))
                throw std::runtime_error("running Sart ELF differs from the installed ELF");
        ...
        void validate_plan_structure(const InstallPlan &plan) {
            ...
            for (const auto &operation : plan.operations) {
                if (operation.path == "/usr/bin/sart" && operation.source.kind == PlanSourceKind::sart_elf) {
                    if (binary != nullptr)
                        throw std::runtime_error("install plan contains duplicate Sart ELF operations");
                    binary = &operation;
                }
            }
            if (binary == nullptr)
                throw std::runtime_error("install plan has no Sart ELF operation");
        ...
        if (!manifest->image) {
            ...
            if (modified.empty()) {
                report.image_verification =
                    std::format("verified active-sha256={} known-good-sha256={} sart-sha256={}", image.active_digest,
                                image.known_good_digest, image.sart_digest);
            } else {
    ...
    ApplyOutcome Installer::apply_exact(const InstallPlan &plan, const ExactInstallDiscovery &discovery) {
        ...
        const auto expected_sart = read_running_elf();
        const auto expected_sart_digest = sha256(expected_sart);
        if (!std::ranges::any_of(plan.operations, [&](const PlanOperation &operation) {
                return operation.path == "/usr/bin/sart" && operation.digest == expected_sart_digest &&
                       operation.content == expected_sart;
            })) {
        ...
        if (const auto existing = read_manifest(root_, expected_owner_uid_)) {
            ...
            try {
                ...
                const auto archive = std::visit(
                    ...
                    discovery.contract);
                const auto inspection = inspect_mkinitfs_boot_deploy_archive(archive, expected_sart);
        ...
        try {
            ...
            std::visit(
                [&](const auto &contract) {
                    ...
                    if constexpr (std::is_same_v<Contract, MkinitfsOpenRcContract>) {
                        inspection = inspect_mkinitfs_openrc_archive(candidate, expected_sart);
                        direct = true;
                        ...
                        inspection = inspect_mkinitfs_boot_deploy_archive(archive, expected_sart);
                        direct = true;
                    }
                },
                discovery.contract);
            ...
            std::visit(
                [&](const auto &contract) {
                    ...
                    if constexpr (std::is_same_v<Contract, DracutSystemdContract>) {
                        image_record =
                            verified_dracut_systemd_image_record(contract, candidate, inspection, expected_sart);
                    } else if constexpr (std::is_same_v<Contract, InitramfsToolsSystemdContract>) {
                        image_record = verified_initramfs_tools_systemd_image_record(contract, candidate, inspection,
                                                                                     expected_sart);
                    } else if constexpr (std::is_same_v<Contract, MkinitcpioSystemdContract>) {
                        image_record =
                            verified_mkinitcpio_systemd_image_record(contract, candidate, inspection, expected_sart);
                    } else if constexpr (std::is_same_v<Contract, MkinitfsOpenRcContract>) {
                        image_record =
                            verified_mkinitfs_openrc_image_record(contract, candidate, inspection, expected_sart);
                    } else {
                        image_record =
                            verified_mkinitfs_boot_deploy_image_record(contract, candidate, inspection, expected_sart);
                    }
                },
                discovery.contract);
            if (android_boot) {
            ...
            if (boot_update && boot_config_index) {
                ...
                if (updated.empty() || !updated_text.contains("sart-known-good")) {
    ...
    UninstallReport Installer::uninstall(const ExactInstallDiscovery *discovery) {
        ...
        try {
            ...
            if (clean_dracut) {
                ...
                write_journal(root_, journal);
                static_cast<void>(run_generator(dracut_systemd_sart_free_generate_request(*manifest->image)));
                ...
                if (clean_candidate.empty() || (clean_candidate_mode & 0022) != 0)
                    throw std::runtime_error("Sart-free dracut candidate is unsafe");
                ...
                static_cast<void>(
                    run_generator(dracut_systemd_sart_free_unpack_request(*manifest->image, journal.transaction)));
                static_cast<void>(inspect_sart_free_dracut_inventory(
                    collect_unpacked_archive_inventory(inspection_host.string(), expected_owner_uid_)));
                std::error_code cleanup_error;
                std::filesystem::remove_all(inspection_host, cleanup_error);
                if (cleanup_error)
            ...
            if (clean_boot_deploy) {
                constexpr std::array<std::string_view, 5> clean_inputs{
                    "/etc/mkinitfs/files-extra/sart", "/etc/mkinitfs/hooks-extra/50-sart-start.sh",
                    "/etc/mkinitfs/hooks-cleanup/90-sart-handoff.sh", "/usr/share/initramfs/init_functions_2nd.sh",
                    "/etc/kernel-cmdline.d/90-sart.conf"};
                ...
                if (clean_candidate.empty() || (clean_candidate_mode & 0022) != 0 || clean_loader.empty() ||
                    ...
                    throw std::runtime_error("Sart-free boot-deploy candidate is unsafe");
                }
                candidate_entry.installed_mode = clean_candidate_mode;
                candidate_entry.installed_digest = sha256(clean_candidate);
                candidate_entry.progress = Progress::applied;
                if (boot_image_index) {
                    ...
                    if (clean_boot_image.empty() || (clean_boot_image_mode & 0022) != 0 ||
                        ...
                        throw std::runtime_error("Sart-free Android boot image is unsafe");
                    }
                ...
                static_cast<void>(inspect_sart_free_mkinitfs_boot_deploy_archive(archive));
                ...
                if (loader_kernel != boot_deploy->kernel_image)
                    throw std::runtime_error("Sart-free loader entry selects a different kernel");
                std::istringstream options(loader_options);
                std::string option;
                while (options >> option) {
                    if (option == "sart=0" || option == "rd.sart=0" || option == "sart=1" || option == "rd.sart=1" ||
                        option == "-splash") {
                        throw std::runtime_error("Sart-free loader entry retains a Sart override");
                    }
                }
                if (boot_deploy->android_boot) {
                    std::uint16_t dtb_mode{};
            ...
            if (clean_dracut || clean_boot_deploy) {
                ...
                const auto active_host = host_path(root_, active_path);
                if (rename(candidate_host.c_str(), active_host.c_str()) != 0)
                    throw system_error("activate Sart-free initramfs", active_host);
                fsync_directory(active_host.parent_path());
                active_entry.progress = Progress::applied;
                write_journal(root_, journal);
            }
    ...
    std::string render_status(const StatusReport &report) {
        std::string output =
            std::format("sart install status\nroot: /\ninstalled: {}\n", report.installed ? "true" : "false");
